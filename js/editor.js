...
function HookCollection() {}! function() {
	function a(a) {
		var b = function(a, b) {
				return e("", a, b)
			},
			f = c;
		a && (c[a] || (c[a] = {}), f = c[a]), f.define && f.define.packaged || (d.original = f.define, f.define = d, f.define.packaged = !0), f.require && f.require.packaged || (e.original = f.require, f.require = b, f.require.packaged = !0)
	}
	var b = "ace",
		c = function() {
			return this
		}();
	if (b || "undefined" == typeof requirejs) {
		var d = function(a, b, c) {
				return "string" != typeof a ? void(d.original ? d.original.apply(window, arguments) : (console.error("dropping module because define wasn't a string."), console.trace())) : (2 == arguments.length && (c = b), d.modules || (d.modules = {}, d.payloads = {}), d.payloads[a] = c, void(d.modules[a] = null))
			},
			e = function(a, b, c) {
				if ("[object Array]" === Object.prototype.toString.call(b)) {
					for (var d = [], f = 0, h = b.length; h > f; ++f) {
						var i = g(a, b[f]);
				...
				if (!c) {
					if (c = d.payloads[b], "function" == typeof c) {
						var g = {},
							...
							j = c(i, g, h);
						g = j || h.exports, d.modules[b] = g, delete d.payloads[b]
					}
					c = d.modules[b] = g || c
				}
				return c
	...
	var c = a("./lib/dom"),
		d = a("./lib/event"),
		e = a("./editor").Editor,
		f = a("./edit_session").EditSession,
		g = a("./undomanager").UndoManager,
		h = a("./virtual_renderer").VirtualRenderer,
		i = a("./multi_select").MultiSelect;
	a("./worker/worker_client"), a("./keyboard/hash_handler"), a("./placeholder"), a("./mode/folding/fold_mode"), a("./theme/textmate"), a("./ext/error_marker"), b.config = a("./config"), b.require = a, b.edit = function(a) {
		if ("string" == typeof a) {
			var f = a,
				a = document.getElementById(f);
			if (!a) throw new Error("ace.edit can't find div #" + f)
		}
		if (a.env && a.env.editor instanceof e) return a.env.editor;
		var g = b.createEditSession(c.getInnerText(a));
		a.innerHTML = "";
		var j = new e(new h(a));
		new i(j), j.setSession(g);
		var k = {
			document: g,
			editor: j,
			onResize: j.resize.bind(j, null)
		};
		return d.addListener(window, "resize", k.onResize), j.on("destroy", function() {
			d.removeListener(window, "resize", k.onResize)
		}), a.env = j.env = k, j
	}, b.createEditSession = function(a, b) {
		var c = new f(a, b);
		return c.setUndoManager(new g), c
	}, b.EditSession = f, b.UndoManager = g
}), ace.define("ace/mode/behaviour", ["require", "exports", "module"], function(a, b) {
	var c = function() {
		this.$behaviours = {}
	};
	(function() {
...
}), ace.define("ace/unicode", ["require", "exports", "module"], function(a, b) {
	function c(a) {
		var c = /\w{4}/g;
...
}), ace.define("ace/token_iterator", ["require", "exports", "module"], function(a, b) {
...
}), ace.define("ace/mode/text_highlight_rules", ["require", "exports", "module", "ace/lib/lang"], function(a, b) {
	var c = a("../lib/lang"),
		d = function() {
			this.$rules = {
				start: [{
	...
	(function() {
		this.addRules = function(a, b) {
			if (b)
				for (var c in a) {
					for (var d = a[c], e = 0; e < d.length; e++) {
						var f = d[e];
						f.next && ("string" != typeof f.next ? f.nextState && 0 !== f.nextState.indexOf(b) && (f.nextState = b + f.nextState) : 0 !== f.next.indexOf(b) && (f.next = b + f.next))
					}
					this.$rules[b + c] = d
				} else
			...
			} if (this.addRules(g, b), d)
				for (var j = Array.prototype[f ? "push" : "unshift"], h = 0; h < e.length; h++) j.apply(this.$rules[e[h]], c.deepCopy(d));
		...
		this.normalizeRules = function() {
			function c(f) {
				...
				for (var h = 0; h < g.length; h++) {
					...
					} else "pop" == j && (i.next = b); if (i.push && (i.nextState = i.next || i.push, i.next = a, delete i.push), i.rules)
...
}), ace.define("ace/anchor", ["require", "exports", "module", "ace/lib/oop", "ace/lib/event_emitter"], function(a, b) {
	var c = a("./lib/oop"),
		d = a("./lib/event_emitter").EventEmitter,
		e = b.Anchor = function(a, b, c) {
		...
		c.implement(this, d), this.getPosition = function() {
			...
			if ((c.start.row != c.end.row || c.start.row == this.row) && !(c.start.row > this.row || c.start.row == this.row && c.start.column > this.column)) {
				var d = this.row,
					...
					f = c.start,
					g = c.end;
				"insertText" === b.action ? f.row === d && f.column <= e ? f.column === e && this.$insertRight || (f.row === g.row ? e += g.column - f.column : (e -= f.column, d += g.row - f.row)) : f.row !== g.row && f.row < d && (d += g.row - f.row) : "insertLines" === b.action ? f.row <= d && (d += g.row - f.row) : "removeText" === b.action ? f.row === d && f.column < e ? e = g.column >= e ? f.column : Math.max(0, e - (g.column - f.column)) : f.row !== g.row && f.row < d ? (g.row === d && (e = Math.max(0, e - g.column) + f.column), d -= g.row - f.row) : g.row === d && (d -= g.row - f.row, e = Math.max(0, e - g.column) + f.column) : "removeLines" == b.action && f.row <= d && (g.row <= d ? d -= g.row - f.row : (d = f.row, e = 0)), this.setPosition(d, e, !0)
			}
...
}), ace.define("ace/background_tokenizer", ["require", "exports", "module", "ace/lib/oop", "ace/lib/event_emitter"], function(a, b) {
	var c = a("./lib/oop"),
		d = a("./lib/event_emitter").EventEmitter,
		e = function(a) {
			...
			this.$worker = function() {
				if (b.running) {
					...
					for (b.running = !1; g > c;) {
						...
						if (h++, h % 5 == 0 && new Date - a > 20) return b.running = setTimeout(b.$worker, 20), void(b.currentLine = c)
					}
	...
	(function() {
		c.implement(this, d), this.setTokenizer = function(a) {
			...
			var b = a.range,
				c = b.start.row,
				d = b.end.row - c;
			if (0 === d) this.lines[c] = null;
			else if ("removeText" == a.action || "removeLines" == a.action) this.lines.splice(c, d + 1, null), this.states.splice(c, d + 1, null);
			else {
				var e = Array(d + 1);
...
}), ace.define("ace/search_highlight", ["require", "exports", "module", "ace/lib/lang", "ace/lib/oop", "ace/range"], function(a, b) {
	var c = a("./lib/lang"),
		d = (a("./lib/oop"), a("./range").Range),
		e = function(a, b, c) {
...
}), ace.define("ace/edit_session/folding", ["require", "exports", "module", "ace/range", "ace/edit_session/fold_line", "ace/edit_session/fold", "ace/token_iterator"], function(a, b) {
	function c() {
		this.getFoldAt = function(a, b, c) {
			...
			if (l && !l.range.isStart(h, i) || m && !m.range.isEnd(j, k)) throw new Error("A fold can't intersect already existing fold" + c.range + l.range);
			...
			var c, e;
			if (null == a ? (c = new d(0, 0, this.getLength(), 0), b = !0) : c = "number" == typeof a ? new d(a, 0, a, this.getLine(a).length) : "row" in a ? d.fromPoints(a, a) : a, e = this.getFoldsInRangeList(c), b) this.removeFolds(e);
			else
				for (var f = e; f.length;) this.expandFolds(f), f = this.getFoldsInRangeList(c);
			...
			null == d && (d = a.start.row, e = 0), null == b && (b = a.end.row, c = this.getLine(b).length);
			var f = this.doc,
				g = "";
			...
			if (this.$foldMode != a) {
				if (this.$foldMode = a, this.removeListener("change", this.$updateFoldWidgets), this._emit("changeAnnotation"), !a || "manual" == this.$foldStyle) return void(this.foldWidgets = null);
			...
			if (this.getFoldWidget) {
				...
				} else b.children ? (j = g ? g.end.row : this.getLength(), this.foldAll(a + 1, g.end.row, b.all ? 1e4 : 0)) : g && (b.all && (g.collapseChildren = 1e4), this.addFold("...", g));
				return g
			}
			...
			var b = a.data,
				...
				d = c.start.row,
				e = c.end.row - d;
			if (0 === e) this.foldWidgets[d] = null;
			else if ("removeText" == b.action || "removeLines" == b.action) this.foldWidgets.splice(d, e + 1, null);
			else {
				var f = Array(e + 1);
	...
	b.Folding = c
}), ace.define("ace/edit_session/fold_line", ["require", "exports", "module", "ace/range"], function(a, b) {
	function c(a, b) {
	...
	(function() {
		this.shiftRow = function(a) {
			...
			var d = this.getNextFoldTo(a, b).fold,
				e = this.folds,
				f = this.foldData;
			if (!d) return null;
			var g = e.indexOf(d),
				h = e[g - 1];
			this.end.row = h.end.row, this.end.column = h.end.column, e = e.splice(g, e.length - g);
			var i = new c(f, e);
			...
			for (var b, c = 0, d = 0; d < this.folds.length; d++) {
				var b = this.folds[d];
				if (a -= b.start.column - c, 0 > a) return {
					row: b.start.row,
					column: b.start.column + a
				};
				if (a -= b.placeholder.length, 0 > a) return b.start;
				c = b.end.column
			}
			return {
				row: this.end.row,
...
}), ace.define("ace/tokenizer", ["require", "exports", "module"], function(a, b) {
	var c = 1e3,
		d = function(a) {
				...
				for (var c = this.states[b], d = [], e = 0, f = this.matchMappings[b] = {
					...
					if (j.defaultToken && (f.defaultToken = j.defaultToken), j.caseInsensitive && (g = "gi"), null != j.regex) {
						...
						if (Array.isArray(j.token))
							if (1 == j.token.length || 1 == l) j.token = j.token[0];
							else {
								if (l - 1 != j.token.length) throw new Error("number of classes and regexp groups in '" + j.token + "'\n'" + j.regex + "' doesn't match\n" + (l - 1) + "!=" + j.token.length);
								j.tokenArray = j.token, j.token = null, j.onMatch = this.$arrayTokens
							} else "function" != typeof j.token || j.onMatch || (j.onMatch = l > 1 ? this.$applyToken : j.token);
						l > 1 && (/\\\d/.test(j.regex) ? k = j.regex.replace(/\\([0-9]+)/g, function(a, b) {
	...
	(function() {
		this.$setMaxTokenCount = function(a) {
			...
			for (var i, j = [], k = 0, l = {
				...
				var m = g.defaultToken,
					n = null,
					o = i[0],
					p = h.lastIndex;
				if (p - o.length > k) {
					var q = a.substring(k, p - o.length);
					l.type == m ? l.value += q : (l.type && j.push(l), l = {
						type: m,
						value: q
					})
				}
				for (var r = 0; r < i.length - 2; r++)
					if (void 0 !== i[r + 1]) {
						n = f[g[r]], m = n.onMatch ? n.onMatch(o, e, d) : n.token, n.next && (e = "string" == typeof n.next ? n.next : n.next(e, d), f = this.states[e], f || (window.console && console.error && console.error(e, "doesn't exist"), e = "start", f = this.states[e]), g = this.matchMappings[e], k = p, h = this.regExps[e], h.lastIndex = p);
						break
					}
				if (o)
					if ("string" == typeof m) n && n.merge === !1 || l.type !== m ? (l.type && j.push(l), l = {
						type: m,
						value: o
					}) : l.value += o;
					else if (m) {
				...
				if (k == a.length) break;
				if (k = p, j.length > c) {
					for (; k < a.length;) l.type && j.push(l), l = {
			...
			return l.type && j.push(l), d.length > 1 && d[0] !== e && d.unshift(e), {
				tokens: j,
				state: d.length ? d : e
			}
		}
	}).call(d.prototype), b.Tokenizer = d
}), ace.define("ace/edit_session/fold", ["require", "exports", "module", "ace/range", "ace/range_list", "ace/lib/oop"], function(a, b) {
	function c(a, b) {
	...
	var g = (a("../range").Range, a("../range_list").RangeList),
		h = a("../lib/oop"),
		i = b.Fold = function(a, b) {
...
}), ace.define("ace/mode/text", ["require", "exports", "module", "ace/tokenizer", "ace/mode/text_highlight_rules", "ace/mode/behaviour", "ace/unicode", "ace/lib/lang", "ace/token_iterator", "ace/range"], function(a, b) {
	var c = a("../tokenizer").Tokenizer,
	...
	(function() {
		this.tokenRe = new RegExp("^[" + f.packages.L + f.packages.Mn + f.packages.Mc + f.packages.Nd + f.packages.Pc + "\\$_]+", "g"), this.nonTokenRe = new RegExp("^(?:[^" + f.packages.L + f.packages.Mn + f.packages.Mc + f.packages.Nd + f.packages.Pc + "\\$_]|\\s])+", "g"), this.getTokenizer = function() {
			return this.$tokenizer || (this.$highlightRules = new this.HighlightRules, this.$tokenizer = new c(this.$highlightRules.getRules())), this.$tokenizer
			...
			if (this.lineCommentStart) {
				if (Array.isArray(this.lineCommentStart)) var m = this.lineCommentStart.map(g.escapeRegExp).join("|"),
					n = this.lineCommentStart[0];
				else var m = g.escapeRegExp(this.lineCommentStart),
				...
				var n = this.blockComment.start,
					...
					m = new RegExp("^(\\s*)(?:" + g.escapeRegExp(n) + ")"),
					u = new RegExp("(?:" + g.escapeRegExp(t) + ")\\s*$"),
					q = function(a, b) {
			...
			if (e) {
				!e.start && e[0] && (e = e[0]);
				var f, g, j = new h(b, d.row, d.column),
					k = j.getCurrentToken(),
				...
				if (k && /comment/.test(k.type)) {
					for (var m, n; k && /comment/.test(k.type);) {
						...
						if (-1 != o) {
							var p = j.getCurrentTokenRow(),
								q = j.getCurrentTokenColumn() + o;
			...
			return !1
		}, this.autoOutdent = function() {}, this.$getIndent = function(a) {
			return a.match(/^\s*/)[0]
...
}), ace.define("ace/range_list", ["require", "exports", "module", "ace/range"], function(a, b) {
	var c = a("./range").Range,
		d = c.comparePoints,
		e = function() {
			this.ranges = []
	...
	(function() {
		this.comparePoints = d, this.pointIndex = function(a, b, c) {
			...
			var b = a.data.range;
			if ("i" == a.data.action[0]) var c = b.start,
				d = b.end;
			else var d = b.start,
				c = b.end;
...
}), ace.define("ace/range", ["require", "exports", "module"], function(a, b) {
	var c = function(a, b) {
	...
	(function() {
		this.isEqual = function(a) {
			...
			else if (this.start.row < a) var e = {
				row: a,
				column: 0
			};
...
}), ace.define("ace/edit_session/bracket_match", ["require", "exports", "module", "ace/token_iterator", "ace/range"], function(a, b) {
	function c() {
	...
	var d = a("../token_iterator").TokenIterator,
		e = a("../range").Range;
	b.BracketMatch = c
}), ace.define("ace/selection", ["require", "exports", "module", "ace/lib/oop", "ace/lib/lang", "ace/lib/event_emitter", "ace/range"], function(a, b) {
	var c = a("./lib/oop"),
		d = a("./lib/lang"),
...
}), ace.define("ace/search", ["require", "exports", "module", "ace/lib/lang", "ace/lib/oop", "ace/range"], function(a, b) {
	var c = a("./lib/lang"),
		d = a("./lib/oop"),
		e = a("./range").Range,
		f = function() {
	...
	(function() {
		this.set = function(a) {
			...
			var d = b.range,
				f = d ? a.getLines(d.start.row, d.end.row) : a.doc.getAllLines(),
				g = [],
				h = b.re;
			if (b.$isMultiLine) {
				var i, j = h.length,
				...
				a: for (var l = h.offset || 0; k >= l; l++) {
					for (var m = 0; j > m; m++)
						if (-1 == f[l + m].search(h[m])) continue a;
					var n = f[l],
						o = f[l + j - 1],
				...
				for (var r = 0; r < f.length; r++)
					for (var s = c.getMatchOffsets(f[r], h), m = 0; m < s.length; m++) {
						var t = s[m];
			...
			var f, g = this,
				h = b.backwards;
			if (b.$isMultiLine) var i = d.length,
				j = function(b, c, g) {
					var h = b.search(d[0]);
					if (-1 != h) {
						for (var j = 1; i > j; j++)
							if (b = a.getLine(c + j), -1 == b.search(d[j])) return;
						var k = b.match(d[i - 1])[0].length,
							l = new e(c, h, c + i - 1, k);
						return 1 == d.offset ? (l.start.row--, l.start.column = Number.MAX_VALUE) : g && (l.start.column += g), f(l) ? !0 : void 0
					}
				};
			else if (h) var j = function(a, b, e) {
				for (var g = c.getMatchOffsets(a, d), h = g.length - 1; h >= 0; h--)
					if (f(g[h], b, e)) return !0
			};
			else var j = function(a, b, e) {
				for (var g = c.getMatchOffsets(a, d), h = 0; h < g.length; h++)
					if (f(g[h], b, e)) return !0
			};
			return {
				forEach: function(c) {
					f = c, g.$lineIterator(a, b).forEach(j)
				}
			}
...
}), ace.define("ace/edit_session", ["require", "exports", "module", "ace/lib/oop", "ace/lib/lang", "ace/config", "ace/lib/event_emitter", "ace/selection", "ace/mode/text", "ace/range", "ace/document", "ace/background_tokenizer", "ace/search_highlight", "ace/edit_session/folding", "ace/edit_session/bracket_match"], function(a, b) {
	var c = a("./lib/oop"),
		d = a("./lib/lang"),
		e = a("./config"),
		f = a("./lib/event_emitter").EventEmitter,
		g = a("./selection").Selection,
		h = a("./mode/text").Mode,
		i = a("./range").Range,
		j = a("./document").Document,
		k = a("./background_tokenizer").BackgroundTokenizer,
		l = a("./search_highlight").SearchHighlight,
		m = function(a, b) {
	...
	(function() {
		...
		c.implement(this, f), this.setDocument = function(a) {
			...
			this.$fromUndo || !this.$undoManager || b.ignore || (this.$deltasDoc.push(b), c && 0 != c.length && this.$deltasFold.push({
				action: "removeFolds",
				folds: c
			}), this.$informUndoManager.schedule()), this.bgTokenizer.$updateOnChange(b), this._signal("change", a)
			...
			return this.$modes["ace/mode/text"] || (this.$modes["ace/mode/text"] = new h), this.$modes[d] && !c ? (this.$onChangeMode(this.$modes[d]), void(b && b())) : (this.$modeId = d, e.loadModule(["mode", d], function(a) {
				return this.$modeId !== d ? b && b() : this.$modes[d] && !c ? this.$onChangeMode(this.$modes[d]) : void(a && a.Mode && (a = new a.Mode(c), c || (this.$modes[d] = a, a.$id = d), this.$onChangeMode(a), b && b()))
			...
			if (b || (this.$modeId = a.$id), this.$mode !== a) {
				...
				this.bgTokenizer.setDocument(this.getDocument()), this.tokenRe = a.tokenRe, this.nonTokenRe = a.nonTokenRe, b || (this.$options.wrapMethod.set.call(this, this.$wrapMethod), this.$setFolding(a.foldingRules), this.bgTokenizer.start(0), this._emit("changeMode"))
			}
		}, this.$stopWorker = function() {
			this.$worker && this.$worker.terminate(), this.$worker = null
		}, this.$startWorker = function() {
			if ("undefined" == typeof Worker || a.noWorker) this.$worker = null;
			else try {
			...
			var f = new i(a, 0, b, Number.MAX_VALUE),
			...
			var b, c = this.$useWrapMode,
				...
				h = a.data.range.end,
				i = null;
			if (-1 != d.indexOf("Lines") ? (f = "insertLines" == d ? e + a.data.lines.length : e, b = a.data.lines ? a.data.lines.length : f - e) : b = f - e, this.$updating = !0, 0 != b)
				if (-1 != d.indexOf("remove")) {
					...
					if (k) {
						...
						0 == p ? (k = k.split(g.row, g.column), k.shiftRow(b), k.addRemoveChars(f, 0, h.column - g.column)) : -1 == p && (k.addRemoveChars(e, 0, h.column - g.column), k.shiftRow(b)), l = j.indexOf(k) + 1
					}
					for (l; l < j.length; l++) {
						var k = j[l];
			...
			for (b = Math.min(b, e.length - 1); b >= i;) d = this.getFoldLine(i, d), d ? (c = [], d.walk(function(a, b, d, f) {
				var g;
				if (null != a) {
					g = this.$getDisplayTokens(a, c.length), g[0] = m;
					for (var h = 1; h < g.length; h++) g[h] = n
		...
		this.$computeWrapSplits = function(a, b) {
			...
			for (var d = [], e = a.length, f = 0, g = 0, h = this.$wrapAsCode; e - f > b;) {
				var i = f + b;
				if (a[i - 1] >= p && a[i] >= p) c(i);
				else if (a[i] != m && a[i] != n) {
					for (var j = Math.max(b - 6, f - 1); i > j && a[i] < m;) i--;
					if (h) {
						for (; i > j && a[i] < m;) i--;
						for (; i > j && a[i] == o;) i--
					} else
			...
			for (var f = 0; f < a.length; f++) {
				...
				if (9 == h) {
					d = this.getScreenTabSize(e.length + c), e.push(q);
					for (var i = 1; d > i; i++) e.push(r)
				} else 32 == h ? e.push(p) : h > 39 && 48 > h || h > 57 && 64 > h ? e.push(o) : h >= 4352 && b(h) ? e.push(p, j) : e.push(g)
			}
			return e
		}, this.$getStringScreenWidth = function(a, c, d) {
			if (0 == c) return [0, 0];
			null == c && (c = 1 / 0), d = d || 0;
			var e, f;
			for (f = 0; f < a.length && (e = a.charCodeAt(f), d += 9 == e ? this.getScreenTabSize(d) : e >= 4352 && b(e) ? 2 : 1, !(d > c)); f++);
			return [d, f]
...
}), ace.define("ace/commands/command_manager", ["require", "exports", "module", "ace/lib/oop", "ace/keyboard/hash_handler", "ace/lib/event_emitter"], function(a, b) {
	var c = a("../lib/oop"),
		d = a("../keyboard/hash_handler").HashHandler,
		e = a("../lib/event_emitter").EventEmitter,
		f = function(a, b) {
	...
	function() {
		c.implement(this, e), this.exec = function(a, b, c) {
			...
			var d = {
				...
				e = this._emit("exec", d);
			return this._signal("afterExec", d), e === !1 ? !1 : !0
...
}), ace.define("ace/keyboard/keybinding", ["require", "exports", "module", "ace/lib/keys", "ace/lib/event"], function(a, b) {
	var c = a("../lib/keys"),
		d = a("../lib/event"),
		e = function(a) {
	...
	(function() {
		this.setDefaultHandler = function(a) {
			...
			for (var f, g = !1, h = this.$editor.commands, i = this.$handlers.length; i-- && (f = this.$handlers[i].handleKeyboard(this.$data, a, b, c, e), !(f && f.command && (g = "null" != f.command || f.passEvent ? h.exec(f.command, this.$editor, f.args, e) : !0, g && e && -1 != a && 1 != f.passEvent && 1 != f.command.passEvent && d.stopEvent(e), g))););
			return g
...
}), ace.define("ace/keyboard/hash_handler", ["require", "exports", "module", "ace/lib/keys", "ace/lib/useragent"], function(a, b) {
	function c(a, b) {
		if (this.platform = b || (e.isMac ? "mac" : "win"), this.commands = {}, this.commandKeyBinding = {}, this.__defineGetter__ && this.__defineSetter__ && "undefined" != typeof console && console.error) {
			var c = !1,
				d = function() {
					c || (c = !0, console.error("commmandKeyBinding has too many m's. use commandKeyBinding"))
				};
			this.__defineGetter__("commmandKeyBinding", function() {
				return d(), this.commandKeyBinding
			}), this.__defineSetter__("commmandKeyBinding", function(a) {
				return d(), this.commandKeyBinding = a
			})
	...
	(function() {
		this.addCommand = function(a) {
			...
			var b = "string" == typeof a ? a : a.name;
			a = this.commands[b], delete this.commands[b];
			var c = this.commandKeyBinding;
			for (var d in c)
				for (var e in c[d]) c[d][e] == a && delete c[d][e]
		}, this.bindKey = function(a, b) {
			if (a) {
				if ("function" == typeof b) return void this.addCommand({
					exec: b,
					bindKey: a,
					name: b.name || a
				});
			...
			var b = a.bindKey;
			if (b) {
				var c = "string" == typeof b ? b : b[this.platform];
				this.bindKey(c, a)
			}
		}, this.parseKeys = function(a) {
			-1 != a.indexOf(" ") && (a = a.split(/\s+/).pop());
	...
	}).call(c.prototype), b.HashHandler = c
}), ace.define("ace/mouse/fold_handler", ["require", "exports", "module"], function(a, b) {
	function c(a) {
	...
	b.FoldHandler = c
}), ace.define("ace/commands/default_commands", ["require", "exports", "module", "ace/lib/lang", "ace/config"], function(a, b) {
	function c(a, b) {
		return {
			win: a,
			mac: b
	...
	b.commands = [{
		...
		bindKey: c("Alt-Shift-E", "Ctrl-Shift-E"),
		exec: function(a) {
			e.loadModule("ace/ext/error_marker", function(b) {
				b.showErrorMarker(a, -1)
			})
		...
		name: "fold",
		bindKey: c("Alt-L|Ctrl-F1", "Command-Alt-L|Command-F1"),
		exec: function(a) {
		...
		name: "foldall",
		bindKey: c("Ctrl-Alt-0", "Ctrl-Command-Option-0"),
		exec: function(a) {
			a.session.foldAll()
		},
		scrollIntoView: "center",
		...
		name: "foldOther",
		bindKey: c("Alt-0", "Command-Option-0"),
		exec: function(a) {
		...
		name: "unfoldall",
		bindKey: c("Alt-Shift-0", "Command-Option-Shift-0"),
		exec: function(a) {
			a.session.unfold()
		},
		scrollIntoView: "center",
		...
		name: "findprevious",
		bindKey: c("Ctrl-Shift-K", "Command-Shift-G"),
		exec: function(a) {
			a.findPrevious()
		},
		readOnly: !0
		...
		bindKey: c("Alt-Shift-K", "Ctrl-Shift-G"),
		exec: function(a) {
		...
		name: "selecttostart",
		bindKey: c("Ctrl-Shift-Home", "Command-Shift-Up"),
		exec: function(a) {
		...
		name: "gotostart",
		bindKey: c("Ctrl-Home", "Command-Home|Command-Up"),
		exec: function(a) {
			a.navigateFileStart()
		},
		multiSelectAction: "forEach",
		...
		name: "selecttoend",
		bindKey: c("Ctrl-Shift-End", "Command-Shift-Down"),
		exec: function(a) {
		...
		name: "gotoend",
		bindKey: c("Ctrl-End", "Command-End|Command-Down"),
		exec: function(a) {
			a.navigateFileEnd()
		},
		multiSelectAction: "forEach",
		...
		name: "selectdown",
		bindKey: c("Shift-Down", "Shift-Down"),
		exec: function(a) {
		...
		name: "selectwordleft",
		bindKey: c("Ctrl-Shift-Left", "Option-Shift-Left"),
		exec: function(a) {
		...
		name: "gotowordleft",
		bindKey: c("Ctrl-Left", "Option-Left"),
		exec: function(a) {
			a.navigateWordLeft()
		},
		multiSelectAction: "forEach",
		...
		bindKey: c("Alt-Shift-Left", "Command-Shift-Left"),
		exec: function(a) {
		...
		name: "gotolinestart",
		bindKey: c("Alt-Left|Home", "Command-Left|Home|Ctrl-A"),
		exec: function(a) {
			a.navigateLineStart()
		},
		multiSelectAction: "forEach",
		...
		name: "selectleft",
		bindKey: c("Shift-Left", "Shift-Left"),
		exec: function(a) {
		...
		bindKey: c("Ctrl-Shift-Right", "Option-Shift-Right"),
		exec: function(a) {
		...
		name: "gotowordright",
		bindKey: c("Ctrl-Right", "Option-Right"),
		exec: function(a) {
			a.navigateWordRight()
		},
		multiSelectAction: "forEach",
		...
		bindKey: c("Alt-Shift-Right", "Command-Shift-Right"),
		exec: function(a) {
		...
		name: "gotolineend",
		bindKey: c("Alt-Right|End", "Command-Right|End|Ctrl-E"),
		exec: function(a) {
			a.navigateLineEnd()
		},
		multiSelectAction: "forEach",
		...
		name: "selectright",
		bindKey: c("Shift-Right", "Shift-Right"),
		exec: function(a) {
		...
		name: "pagedown",
		bindKey: c(null, "Option-PageDown"),
		exec: function(a) {
			a.scrollPageDown()
		},
		readOnly: !0
		...
		name: "gotopagedown",
		bindKey: c("PageDown", "PageDown|Ctrl-V"),
		exec: function(a) {
			a.gotoPageDown()
		},
		readOnly: !0
		...
		bindKey: c("Ctrl-Alt-E", "Command-Option-E"),
		exec: function(a) {
		...
		name: "replaymacro",
		bindKey: c("Ctrl-Shift-E", "Command-Shift-E"),
		exec: function(a) {
			a.commands.replay(a)
		},
		readOnly: !0
		...
		bindKey: c("Ctrl-Shift-D", "Command-Shift-D"),
		exec: function(a) {
		...
		name: "sortlines",
		bindKey: c("Ctrl-Alt-S", "Command-Alt-S"),
		exec: function(a) {
			a.sortLines()
		},
		...
		bindKey: c("Ctrl-Shift-/", "Command-Shift-/"),
		exec: function(a) {
		...
		name: "modifyNumberUp",
		bindKey: c("Ctrl-Shift-Up", "Alt-Shift-Up"),
		exec: function(a) {
			a.modifyNumber(1)
		},
		multiSelectAction: "forEach"
		...
		bindKey: c("Ctrl-Shift-Down", "Alt-Shift-Down"),
		exec: function(a) {
			a.modifyNumber(-1)
		},
		multiSelectAction: "forEach"
		...
		name: "replace",
		bindKey: c("Ctrl-H", "Command-Option-F"),
		exec: function(a) {
			e.loadModule("ace/ext/searchbox", function(b) {
				b.Search(a, !0)
			})
		...
		name: "undo",
		bindKey: c("Ctrl-Z", "Command-Z"),
		exec: function(a) {
			a.undo()
		}
	}, {
		name: "redo",
		bindKey: c("Ctrl-Shift-Z|Ctrl-Y", "Command-Shift-Z|Command-Y"),
		exec: function(a) {
			a.redo()
		}
	}, {
		name: "copylinesup",
		bindKey: c("Alt-Shift-Up", "Command-Option-Up"),
		exec: function(a) {
			a.copyLinesUp()
		},
		scrollIntoView: "cursor"
		...
		name: "copylinesdown",
		bindKey: c("Alt-Shift-Down", "Command-Option-Down"),
		exec: function(a) {
			a.copyLinesDown()
		},
		scrollIntoView: "cursor"
		...
		name: "movelinesdown",
		bindKey: c("Alt-Down", "Option-Down"),
		exec: function(a) {
			a.moveLinesDown()
		},
		scrollIntoView: "cursor"
		...
		name: "del",
		bindKey: c("Delete", "Delete|Ctrl-D|Shift-Delete"),
		exec: function(a) {
			a.remove("right")
		},
		multiSelectAction: "forEach",
		...
		bindKey: c("Alt-Backspace", "Command-Backspace"),
		exec: function(a) {
			a.removeToLineStart()
		},
		multiSelectAction: "forEach",
		...
		name: "removewordleft",
		bindKey: c("Ctrl-Backspace", "Alt-Backspace|Ctrl-Alt-Backspace"),
		exec: function(a) {
			a.removeWordLeft()
		},
		multiSelectAction: "forEach",
		...
		bindKey: c("Ctrl-Delete", "Alt-Delete"),
		exec: function(a) {
			a.removeWordRight()
		},
		multiSelectAction: "forEach",
		...
		name: "tolowercase",
		bindKey: c("Ctrl-Shift-U", "Ctrl-Shift-U"),
		exec: function(a) {
			a.toLowerCase()
		},
		multiSelectAction: "forEach",
...
}), ace.define("ace/lib/event_emitter", ["require", "exports", "module"], function(a, b) {
	var c = {},
		d = function() {
...
}), ace.define("ace/undomanager", ["require", "exports", "module"], function(a, b) {
	var c = function() {
		this.reset()
	};
	(function() {
...
}), ace.define("ace/lib/net", ["require", "exports", "module", "ace/lib/dom"], function(a, b) {
	var c = a("./dom");
	b.get = function(a, b) {
	...
	var p = function(a, b) {
		...
		this.canvas = n.element, this.$markerFront = new g(this.content), this.$cursorLayer = new i(this.content), this.$horizScroll = !1, this.$vScroll = !1, this.scrollBar = this.scrollBarV = new k(this.container, this), this.scrollBarH = new j(this.container, this), this.scrollBarV.addEventListener("scroll", function(a) {
			...
			row: 0,
			column: 0
		}, this.$fontMetrics = new m(this.container, 500), this.$textLayer.$setFontMetrics(this.$fontMetrics), this.$textLayer.addEventListener("changeCharacterSize", function(a) {
			...
			v: 0,
			h: 0
		}, this.$loop = new l(this.$renderChanges.bind(this), this.container.ownerDocument.defaultView), this.$loop.schedule(this.CHANGE_FULL), this.updateCharacterSize(), this.setPadding(4), e.resetOptions(this), e._emit("renderer", this)
	};
	(function() {
		this.CHANGE_CURSOR = 1, this.CHANGE_MARKER = 2, this.CHANGE_GUTTER = 4, this.CHANGE_SCROLL = 8, this.CHANGE_LINES = 16, this.CHANGE_TEXT = 32, this.CHANGE_SIZE = 64, this.CHANGE_MARKER_BACK = 128, this.CHANGE_MARKER_FRONT = 256, this.CHANGE_FULL = 512, this.CHANGE_H_SCROLL = 1024, c.implement(this, n), this.updateCharacterSize = function() {
			...
			void 0 === b && (b = 1 / 0), this.$changedLines ? (this.$changedLines.firstRow > a && (this.$changedLines.firstRow = a), this.$changedLines.lastRow < b && (this.$changedLines.lastRow = b)) : this.$changedLines = {
				firstRow: a,
				lastRow: b
			...
			if (!(this.resizing > 2)) {
				...
				a && (this.$gutterLayer.$padding = null), a ? this.$renderChanges(f | this.$changes, !0) : this.$loop.schedule(f | this.$changes), this.resizing && (this.resizing = 0)
			}
			...
			if (this.$keepTextAreaAtCursor) {
				...
				if (!(0 > b || b > a.height - d)) {
					var e = this.characterWidth;
					if (this.$composition) {
						var f = this.textarea.value.replace(/^\x01+/, "");
						e *= this.session.$getStringScreenWidth(f)[0] + 2, d += 2, b -= 1
					}
					c -= this.scrollLeft, c > this.$size.scrollerWidth - e && (c = this.$size.scrollerWidth - e), c -= this.scrollBar.isVisible ? this.scrollBar.width : 0, this.textarea.style.height = d + "px", this.textarea.style.width = e + "px", this.textarea.style.right = Math.max(0, this.$size.scrollerWidth - c - e) + "px", this.textarea.style.bottom = Math.max(0, this.$size.height - b - d) + "px"
				}
			}
			...
			if (c != this.desiredHeight || this.$size.height != this.desiredHeight || d != this.$vScroll) {
				...
				this.container.style.height = c + "px", this.$updateCachedSize(!0, this.$gutterWidth, e, c), this.desiredHeight = c
			}
			...
			var a = this.session,
				...
				g = b.scrollerHeight + this.lineHeight,
				h = this.$getLongestLine(),
				i = !c && (this.$hScrollBarAlwaysVisible || b.scrollerWidth - h - 2 * this.$padding < 0),
				j = this.$horizScroll !== i;
			j && (this.$horizScroll = i, this.scrollBarH.setVisible(i)), !this.$maxLines && this.$scrollPastEnd && this.scrollTop > e - b.scrollerHeight && (e += Math.min((b.scrollerHeight - this.lineHeight) * this.$scrollPastEnd, this.scrollTop - e + b.scrollerHeight));
			var k = !c && (this.$vScrollBarAlwaysVisible || b.scrollerHeight - e < 0),
				l = this.$vScroll !== k;
			l && (this.$vScroll = k, this.scrollBarV.setVisible(k)), this.session.setScrollTop(Math.max(-this.scrollMargin.top, Math.min(this.scrollTop, e - b.scrollerHeight + this.scrollMargin.bottom))), this.session.setScrollLeft(Math.max(-this.scrollMargin.left, Math.min(this.scrollLeft, h + 2 * this.$padding - b.scrollerWidth + this.scrollMargin.right)));
			var m, n, o = Math.ceil(g / this.lineHeight) - 1,
				p = Math.max(0, Math.round((this.scrollTop - f) / this.lineHeight)),
				q = p + o,
				r = this.lineHeight;
			p = a.screenToDocumentRow(p, 0);
			var s = a.getFoldLine(p);
			s && (p = s.start.row), m = a.documentToScreenRow(p, 0), n = a.getRowLength(p) * r, q = Math.min(a.screenToDocumentRow(q, 0), a.getLength() - 1), g = b.scrollerHeight + a.getRowLength(q) * r + n, f = this.scrollTop - m * r;
			var t = 0;
			return this.layerConfig.width != h && (t = this.CHANGE_H_SCROLL), (j || l) && (t = this.$updateCachedSize(!0, this.gutterWidth, b.width, b.height), this._signal("scrollbarVisibilityChanged"), l && (h = this.$getLongestLine())), this.layerConfig = {
				width: h,
				...
				maxHeight: e,
				offset: f,
				gutterOffset: Math.max(0, Math.ceil((f + b.height - b.scrollerHeight) / r)),
			...
			function c(c) {
				...
				if (c.cssClass) {
					...
					f.$padding && e != f.$padding && f.setPadding(e), f.$theme = c.cssClass, f.theme = c, d.addCssClass(f.container, c.cssClass), d.setCssClass(f.container, "ace_dark", c.isDark), f.$size && (f.$size.width = 0, f.$updateSizeAsync()), f._dispatchEvent("themeLoaded", {
						theme: c
					}), b && b()
				}
			}
			var f = this;
			if (this.$themeId = a, f._dispatchEvent("themeChange", {
				theme: a
			...
			this.content.style.cursor != a && (this.content.style.cursor = a)
		}, this.setMouseCursor = function(a) {
			this.content.style.cursor = a
		}, this.destroy = function() {
			this.$textLayer.destroy(), this.$cursorLayer.destroy()
		}
	}).call(p.prototype), e.defineOptions(p.prototype, "renderer", {
		animatedScroll: {
			initialValue: !1
		},
		showInvisibles: {
...
}), ace.define("ace/config", ["require", "exports", "module", "ace/lib/lang", "ace/lib/oop", "ace/lib/net", "ace/lib/event_emitter"], function(a, b, c) {
	"no use strict";

	function d(d) {
		if (k.packaged = d || a.packaged || c.packaged || j.define && define.packaged, !j.document) return "";
		for (var f = {}, g = "", h = document.getElementsByTagName("script"), i = 0; i < h.length; i++) {
			var l = h[i],
				m = l.src || l.getAttribute("src");
			if (m) {
				for (var n = l.attributes, o = 0, p = n.length; p > o; o++) {
					var q = n[o];
					0 === q.name.indexOf("data-ace-") && (f[e(q.name.replace(/^data-ace-/, ""))] = q.value)
				}
				var r = m.match(/^(.*)\/ace(\-\w+)?\.js(\?|$)/);
				r && (g = r[1])
			}
		}
		...
		for (var s in f) "undefined" != typeof f[s] && b.set(s, f[s])
	}
	...
	var f = a("./lib/lang"),
		g = a("./lib/oop"),
		h = a("./lib/net"),
		i = a("./lib/event_emitter").EventEmitter,
		j = function() {
			return this
		}(),
		k = {
	...
	b.get = function(a) {
		...
		return f.copyObject(k)
	}, g.implement(b, i), b.moduleUrl = function(a, b) {
		...
		var g = k[b + "Path"];
		return null == g ? g = k.basePath : "/" == d && (b = d = ""), g && "/" != g.slice(-1) && (g += "/"), g + b + d + e + this.get("suffix")
		...
		if (b.$loading[c] || (b.$loading[c] = []), b.$loading[c].push(d), !(b.$loading[c].length > 1)) {
			...
			return b.get("packaged") ? void h.loadScript(b.moduleUrl(c, f), i) : i()
		}
	}, d(!0), b.init = d;
	var l = {
			...
			setOption: function(a, b) {
				if (this["$" + a] !== b) {
					...
					if (!c) return void("undefined" != typeof console && console.warn && console.warn('misspelled option "' + a + '"'));
			...
			getOption: function(a) {
				...
				return b ? b.forwardTo ? this[b.forwardTo] && this[b.forwardTo].getOption(a) : b && b.get ? b.get.call(this) : this["$" + a] : void("undefined" != typeof console && console.warn && console.warn('misspelled option "' + a + '"'))
			}
		},
		m = {};
	b.defineOptions = function(a, b, c) {
		return a.$options || (m[b] = a.$options = {}), Object.keys(c).forEach(function(b) {
			var d = c[b];
			"string" == typeof d && (d = {
				forwardTo: d
			}), d.name || (d.name = b), a.$options[d.name] = d, "initialValue" in d && (a["$" + d.name] = d.initialValue)
		}), g.implement(a, l), this
	}, b.resetOptions = function(a) {
		Object.keys(a.$options).forEach(function(b) {
			var c = a.$options[b];
			"value" in c && a.setOption(b, c.value)
		})
	}, b.setDefaultValue = function(a, c, d) {
		var e = m[a] || (m[a] = {});
		e[c] && (e.forwardTo ? b.setDefaultValue(e.forwardTo, c, d) : e[c].value = d)
	}, b.setDefaultValues = function(a, c) {
		Object.keys(c).forEach(function(d) {
			b.setDefaultValue(a, d, c[d])
		})
	}
}), ace.define("ace/layer/gutter", ["require", "exports", "module", "ace/lib/dom", "ace/lib/oop", "ace/lib/lang", "ace/lib/event_emitter"], function(a, b) {
	var c = a("../lib/dom"),
		d = a("../lib/oop"),
		e = a("../lib/lang"),
		f = a("../lib/event_emitter").EventEmitter,
	...
	(function() {
		d.implement(this, f), this.setSession = function(a) {
			this.session && this.session.removeEventListener("change", this.$updateAnnotations), this.session = a, a.on("change", this.$updateAnnotations)
		}, this.addGutterDecoration = function(a, b) {
			...
			for (var b = 0; b < a.length; b++) {
				var c = a[b],
					d = c.row,
					f = this.$annotations[d];
				f || (f = this.$annotations[d] = {
					text: []
				});
				var g = c.text;
			...
			if (this.$annotations.length) {
				var b = a.data,
					...
					e = c.end.row - d;
				if (0 === e);
				else if ("removeText" == b.action || "removeLines" == b.action) this.$annotations.splice(d, e + 1, null);
				else {
					var f = new Array(e + 1);
...
}), ace.define("ace/mouse/dragdrop_handler", ["require", "exports", "module", "ace/lib/dom", "ace/lib/event", "ace/lib/useragent"], function(a, b) {
	function c(a) {
		function b(a, b) {
			var c = Date.now(),
				e = !b || a.row != b.row,
		...
		function l() {
			x = q.selection.toOrientedRange(), t = q.session.addMarker(x, "ace_selection", q.getSelectionStyle()), q.clearSelection(), q.isFocused() && q.renderer.$cursorLayer.setBlinking(!1), clearInterval(w), w = setInterval(k, 20), F = 0, f.addListener(document, "mousemove", n)
		}

		function m() {
			clearInterval(w), q.session.removeMarker(t), t = null, q.$blockScrolling += 1, q.selection.fromOrientedRange(x), q.$blockScrolling -= 1, q.isFocused() && !A && q.renderer.$cursorLayer.setBlinking(!q.getReadOnly()), x = null, F = 0, B = null, C = null, f.removeListener(document, "mousemove", n)
		}
		...
		this.onDragStart = function(a) {
			...
			c.effectAllowed = q.getReadOnly() ? "copy" : "copyMove", g.isOpera && (q.container.appendChild(r), r._top = r.offsetTop), c.setDragImage && c.setDragImage(r, 0, 0), g.isOpera && q.container.removeChild(r), c.clearData(), c.setData("Text", q.session.getTextRange()), A = !0, this.setState("drag")
			...
			return !q.getReadOnly() && o(a.dataTransfer) ? (t || l(), F++, a.dataTransfer.dropEffect = z = p(a), f.preventDefault(a)) : void 0
		}, this.onDragOver = function(a) {
			return !q.getReadOnly() && o(a.dataTransfer) ? (t || (l(), F++), null !== G && (G = null), u = a.clientX, v = a.clientY, a.dataTransfer.dropEffect = z = p(a), f.preventDefault(a)) : void 0
	...
	(function() {
		this.dragWait = function() {
			...
			this.cancelDrag = !1;
			var a = this.editor.container;
			a.draggable = !0, this.editor.renderer.$cursorLayer.setBlinking(!1), this.editor.setStyle("ace_dragging"), this.setState("dragReady")
...
}), ace.define("ace/layer/marker", ["require", "exports", "module", "ace/range", "ace/lib/dom"], function(a, b) {
	var c = a("../range").Range,
		d = a("../lib/dom"),
		e = function(a) {
		...
		this.$padding = 0, this.setPadding = function(a) {
			...
			if (a) {
				...
				for (var c in this.markers) {
					var e = this.markers[c];
					if (e.range) {
						var f = e.range.clipRows(a.firstRow, a.lastRow);
						if (!f.isEmpty())
							if (f = f.toScreenRange(this.session), e.renderer) {
								var g = this.$getTop(f.start.row, a),
									h = this.$padding + f.start.column * a.characterWidth;
								e.renderer(b, f, h, g, a)
							} else "fullLine" == e.type ? this.drawFullLineMarker(b, f, e.clazz, a) : "screenLine" == e.type ? this.drawScreenLineMarker(b, f, e.clazz, a) : f.isMultiLine() ? "text" == e.type ? this.drawTextMarker(b, f, e.clazz, a) : this.drawMultiLineMarker(b, f, e.clazz, a) : this.drawSingleLineMarker(b, f, e.clazz + " ace_start", a)
					} else e.update(b, this, this.session, a)
				}
			...
			var g = b.start.row,
				h = new c(g, b.start.column, g, this.session.getScreenLastRowColumn(g));
...
}), ace.define("ace/mouse/mouse_event", ["require", "exports", "module", "ace/lib/event", "ace/lib/useragent"], function(a, b) {
	var c = a("../lib/event"),
		d = a("../lib/useragent"),
...
}), ace.define("ace/layer/text", ["require", "exports", "module", "ace/lib/oop", "ace/lib/dom", "ace/lib/lang", "ace/lib/useragent", "ace/lib/event_emitter"], function(a, b) {
	var c = a("../lib/oop"),
		d = a("../lib/dom"),
		e = a("../lib/lang"),
		f = (a("../lib/useragent"), a("../lib/event_emitter").EventEmitter),
		g = function(a) {
		...
		c.implement(this, f), this.EOF_CHAR = "¶", this.EOL_CHAR_LF = "¬", this.EOL_CHAR_CRLF = "¤", this.EOL_CHAR = this.EOL_CHAR_LF, this.TAB_CHAR = "→", this.SPACE_CHAR = "·", this.$padding = 0, this.$updateEolChar = function() {
			...
			if (this.displayIndentGuides) {
				...
				if (this.showInvisibles) {
					...
					var h = e.stringRepeat(this.SPACE_CHAR, this.tabSize),
						i = this.TAB_CHAR + e.stringRepeat(" ", this.tabSize - 1)
				} else var h = e.stringRepeat(" ", this.tabSize),
					i = h;
			...
			for (var e = Math.max(b, a.firstRow), f = Math.min(c, a.lastRow), g = this.element.childNodes, h = 0, i = a.firstRow; e > i; i++) {
			...
			for (var i = e, j = this.session.getNextFoldLine(i), k = j ? j.start.row : 1 / 0;;) {
				if (i > k && (i = j.end.row + 1, j = this.session.getNextFoldLine(i, j), k = j ? j.start.row : 1 / 0), i > f) break;
				var l = g[h++];
				if (l) {
					var m = [];
					this.$renderLine(m, i, !this.$useLineGroups(), i == k ? j : !1), l.style.height = a.lineHeight * this.session.getRowLength(i) + "px", d.setInnerHtml(l, m.join(""))
				}
				i++
			}
			...
			for (var e = this.element.ownerDocument.createDocumentFragment(), f = b, g = this.session.getNextFoldLine(f), h = g ? g.start.row : 1 / 0;;) {
				...
				var i = d.createElement("div"),
					j = [];
				if (this.$renderLine(j, f, !1, f == h ? g : !1), i.innerHTML = j.join(""), this.$useLineGroups()) i.className = "ace_line_group", e.appendChild(i), i.style.height = a.lineHeight * this.session.getRowLength(f) + "px";
				else
			...
			this.config = a;
			for (var b = [], c = a.firstRow, e = a.lastRow, f = c, g = this.session.getNextFoldLine(f), h = g ? g.start.row : 1 / 0;;) {
				if (f > h && (f = g.end.row + 1, g = this.session.getNextFoldLine(f, g), h = g ? g.start.row : 1 / 0), f > e) break;
				this.$useLineGroups() && b.push("<div class='ace_line_group' style='height:", a.lineHeight * this.session.getRowLength(f), "px'>"), this.$renderLine(b, f, !1, f == h ? g : !1), this.$useLineGroups() && b.push("</div>"), f++
			}
			this.element = d.setInnerHtml(this.element, b.join(""))
		}, this.$textToken = {
			text: !0,
			...
			var f = this,
				g = /\t|&|<|( +)|([\x00-\x1f\x80-\xa0\u1680\u180E\u2000-\u200f\u2028\u2029\u202F\u205F\u3000\uFEFF])|[\u1100-\u115F\u11A3-\u11A7\u11FA-\u11FF\u2329-\u232A\u2E80-\u2E99\u2E9B-\u2EF3\u2F00-\u2FD5\u2FF0-\u2FFB\u3000-\u303E\u3041-\u3096\u3099-\u30FF\u3105-\u312D\u3131-\u318E\u3190-\u31BA\u31C0-\u31E3\u31F0-\u321E\u3220-\u3247\u3250-\u32FE\u3300-\u4DBF\u4E00-\uA48C\uA490-\uA4C6\uA960-\uA97C\uAC00-\uD7A3\uD7B0-\uD7C6\uD7CB-\uD7FB\uF900-\uFAFF\uFE10-\uFE19\uFE30-\uFE52\uFE54-\uFE66\uFE68-\uFE6B\uFF01-\uFF60\uFFE0-\uFFE6]/g,
				h = function(a, c, d, g) {
					if (c) return f.showInvisibles ? "<span class='ace_invisible ace_invisible_space'>" + e.stringRepeat(f.SPACE_CHAR, a.length) + "</span>" : e.stringRepeat(" ", a.length);
					if ("&" == a) return "&#38;";
					if ("<" == a) return "&#60;";
					if ("	" == a) {
					...
					if ("　" == a) {
						var i = f.showInvisibles ? "ace_cjk ace_invisible ace_invisible_space" : "ace_cjk",
							j = f.showInvisibles ? f.SPACE_CHAR : "";
			...
			else var e = this.session.getTokens(b); if (c || a.push("<div class='ace_line' style='height:", this.config.lineHeight * (this.$useLineGroups() ? 1 : this.session.getRowLength(b)), "px'>"), e.length) {
...
}), ace.define("ace/tooltip", ["require", "exports", "module", "ace/lib/oop", "ace/lib/dom"], function(a, b) {
	function c(a) {
...
}), ace.define("ace/layer/cursor", ["require", "exports", "module", "ace/lib/dom"], function(a, b) {
	var c, d = a("../lib/dom"),
		e = function(a) {
...
}), ace.define("ace/mouse/default_gutter_handler", ["require", "exports", "module", "ace/lib/dom", "ace/lib/oop", "ace/lib/event", "ace/tooltip"], function(a, b) {
	function c(a) {
		function b() {
			...
			if (b == e) {
				var g = h.renderer.pixelToScreenCoordinates(0, l.y).row,
					k = l.$pos;
				if (g > h.session.documentToScreenRow(k.row, k.column)) return c()
			}
			if (m != d)
				if (m = d.text.join("<br/>"), j.setHtml(m), j.show(), h.on("mousewheel", c), a.$tooltipFollowsMouse) f(l);
				else {
					var n = i.$cells[b].element,
...
}), ace.define("ace/scrollbar", ["require", "exports", "module", "ace/lib/oop", "ace/lib/dom", "ace/lib/event", "ace/lib/event_emitter"], function(a, b) {
	var c = a("./lib/oop"),
		d = a("./lib/dom"),
		e = a("./lib/event"),
		f = a("./lib/event_emitter").EventEmitter,
...
}), ace.define("ace/mouse/default_handlers", ["require", "exports", "module", "ace/lib/dom", "ace/lib/event", "ace/lib/useragent"], function(a, b) {
	function c(a) {
		...
		this.onMouseDown = function(a) {
			...
			if (0 !== e) {
				var f = d.getSelectionRange(),
					g = f.isEmpty();
				return g && d.selection.moveToPosition(c), void d.textInput.onContextMenu(a.domEvent)
			}
			return !b || d.isFocused() || (d.focus(), !this.$focusTimout || this.$clickSelection || d.inMultiSelectMode) ? (this.captureMouse(a), !b || this.$clickSelection || a.getShiftKey() || d.inMultiSelectMode ? this.startSelect(c) : b && (this.mousedownEvent.time = Date.now(), this.startSelect(c)), a.preventDefault()) : (this.mousedownEvent.time = Date.now(), this.setState("focusWait"), void this.captureMouse(a))
		}, this.startSelect = function(a) {
			a = a || this.editor.renderer.screenToTextCoordinates(this.x, this.y);
			var b = this.editor,
				c = this.mousedownEvent.getShiftKey();
			c ? b.selection.selectToPosition(a) : this.$clickSelection || b.selection.moveToPosition(a), b.renderer.scroller.setCapture && b.renderer.scroller.setCapture(), b.setStyle("ace_selecting"), this.setState("select")
			...
			var b, c = this.editor,
				...
				f = c.selection[a](d.row, d.column);
			if (this.$clickSelection) {
				var g = this.$clickSelection.comparePoint(f.start),
					h = this.$clickSelection.comparePoint(f.end);
				if (-1 == g && 0 >= h) b = this.$clickSelection.end, (f.end.row != d.row || f.end.column != d.column) && (d = f.start);
				else if (1 == h && g >= 0) b = this.$clickSelection.start, (f.start.row != d.row || f.start.column != d.column) && (d = f.end);
				else if (-1 == g && 1 == h) d = f.end, b = f.start;
				else {
			...
			var a = d(this.mousedownEvent.x, this.mousedownEvent.y, this.x, this.y),
				b = Date.now();
			...
			e ? (e.isEmpty() && (e.start.column--, e.end.column++), this.setState("select")) : (e = c.selection.getWordRange(b.row, b.column), this.setState("selectByWords")), this.$clickSelection = e, this[this.state] && this[this.state](a)
		}, this.onTripleClick = function(a) {
			var b = a.getDocumentPosition(),
				c = this.editor;
			this.setState("selectByLines"), this.$clickSelection = c.selection.getLineRange(b.row), this[this.state] && this[this.state](a)
...
}), ace.define("ace/renderloop", ["require", "exports", "module", "ace/lib/event"], function(a, b) {
	var c = a("./lib/event"),
		d = function(a, b) {
...
}), ace.define("ace/mouse/mouse_handler", ["require", "exports", "module", "ace/lib/event", "ace/lib/useragent", "ace/mouse/default_handlers", "ace/mouse/default_gutter_handler", "ace/mouse/mouse_event", "ace/mouse/dragdrop_handler", "ace/config"], function(a, b) {
	var c = a("../lib/event"),
		d = a("../lib/useragent"),
		e = a("./default_handlers").DefaultHandlers,
		f = a("./default_gutter_handler").GutterHandler,
		g = a("./mouse_event").MouseEvent,
		h = a("./dragdrop_handler").DragdropHandler,
		i = a("../config"),
		j = function(a) {
			this.editor = a, new e(this), new f(this), new h(this);
			var b = a.renderer.getMouseEventTarget();
			c.addListener(b, "click", this.onMouseEvent.bind(this, "click")), c.addListener(b, "mousemove", this.onMouseMove.bind(this, "mousemove")), c.addMultiMouseDownListener(b, [400, 300, 250], this, "onMouseEvent"), a.renderer.scrollBarV && (c.addMultiMouseDownListener(a.renderer.scrollBarV.inner, [400, 300, 250], this, "onMouseEvent"), c.addMultiMouseDownListener(a.renderer.scrollBarH.inner, [400, 300, 250], this, "onMouseEvent")), c.addMouseWheelListener(a.container, this.onMouseWheel.bind(this, "mousewheel"));
			var d = a.renderer.$gutter;
			c.addListener(d, "mousedown", this.onMouseEvent.bind(this, "guttermousedown")), c.addListener(d, "click", this.onMouseEvent.bind(this, "gutterclick")), c.addListener(d, "dblclick", this.onMouseEvent.bind(this, "gutterdblclick")), c.addListener(d, "mousemove", this.onMouseEvent.bind(this, "guttermousemove")), c.addListener(b, "mousedown", function() {
				a.focus()
			}), c.addListener(d, "mousedown", function(b) {
	...
	(function() {
		c.implement(this, f), this.$characterSize = {
			...
			g = b > 0 && 1 > b ? 1 : 100, a.parentNode.removeChild(a)
		}, this.$setMeasureNodeStyles = function(a, b) {
			a.width = a.height = "auto", a.left = a.top = "-100px", a.visibility = "hidden", a.position = "fixed", a.whiteSpace = "pre", a.font = "inherit", a.overflow = b ? "hidden" : "visible"
		}, this.checkForSizeChanges = function() {
			...
			if (1 === g) var a = this.$measureNode.getBoundingClientRect(),
				b = {
					height: a.height,
					width: a.width
				};
				...
				width: this.$measureNode.clientWidth / g
			};
			return 0 === b.width || 0 === b.height ? null : b
		}, this.$measureCharWidth = function(a) {
			this.$main.innerHTML = e.stringRepeat(a, g);
...
}), ace.define("ace/keyboard/textinput", ["require", "exports", "module", "ace/lib/event", "ace/lib/useragent", "ace/lib/dom", "ace/lib/lang"], function(a, b) {
	var c = a("../lib/event"),
		d = a("../lib/useragent"),
		e = a("../lib/dom"),
		f = a("../lib/lang"),
		...
		i = function(a, b) {
			function i(a) {
				if (!p) {
					if (C) b = 0, c = a ? 0 : l.value.length - 1;
					else var b = a ? 2 : 1,
						c = 2;
					try {
			...
			l.className = "ace_text-input", d.isTouchPad && l.setAttribute("x-palm-disable-auto-cap", !0), l.wrap = "off", l.autocorrect = "off", l.autocapitalize = "off", l.spellcheck = !1, l.style.opacity = "0", a.insertBefore(l, a.firstChild);
			var m = "",
				n = !1,
				o = !1,
				p = !1,
			...
			c.addListener(l, "blur", function() {
				b.onBlur(), s = !1
			}), c.addListener(l, "focus", function() {
				s = !0, b.onFocus(), i()
			}), this.focus = function() {
				l.focus()
			}, this.blur = function() {
			...
			var D = !1,
				E = function(a) {
					C && (a = C(a), C = null), o ? (i(), a && b.onPaste(a), o = !1) : a == m.charAt(0) ? D && b.execCommand("del", {
						source: "ace"
				...
				G = function(a, b) {
					...
					if (c && !g) {
						...
						return b ? c.setData(d, b) : c.getData(d)
					}
				},
				H = function(a, d) {
					var e = b.getCopyText();
			...
			var L = function() {
					!p && b.onCompositionStart && (p = {}, b.onCompositionStart(), setTimeout(M, 0), b.on("mousedown", N), b.selection.isEmpty() || (b.insert(""), b.session.markUndoGroup(), b.selection.clearSelection()), b.session.markUndoGroup())
				},
				M = function() {
					if (p && b.onCompositionUpdate) {
			...
			if (c.addListener(l, "compositionstart", L), d.isGecko ? c.addListener(l, "text", function() {
				O.schedule()
				...
				D = !0, q || (q = l.style.cssText), l.style.cssText = "z-index:100000;" + (d.isIE ? "opacity:0.1;" : ""), i(b.selection.isEmpty()), b._emit("nativecontextmenu", {
					target: b,
					domEvent: a
				});
				var f = b.container.getBoundingClientRect(),
					g = e.computedStyle(b.container),
					h = f.top + (parseInt(g.borderTopWidth) || 0),
					j = f.left + (parseInt(f.borderLeftWidth) || 0),
					m = f.bottom - h - l.clientHeight,
					n = function(a) {
						l.style.left = a.clientX - j - 2 + "px", l.style.top = Math.min(a.clientY - h - 2, m) + "px"
					};
				n(a), "mousedown" == a.type && (b.renderer.$keepTextAreaAtCursor && (b.renderer.$keepTextAreaAtCursor = null), d.isWin && c.capture(b.container, n, k))
			}, this.onContextMenuClose = k, !d.isGecko || d.isMac) {
				var P = function(a) {
					b.textInput.onContextMenu(a), k()
				};
				c.addListener(b.renderer.scroller, "contextmenu", P), c.addListener(l, "contextmenu", P)
			}
		};
	b.TextInput = i
}), ace.define("ace/lib/lang", ["require", "exports", "module"], function(a, b) {
	b.last = function(a) {
		return a[a.length - 1]
...
}), ace.define("ace/multi_select", ["require", "exports", "module", "ace/range_list", "ace/range", "ace/selection", "ace/mouse/multi_select_handler", "ace/lib/event", "ace/lib/lang", "ace/commands/multi_select_commands", "ace/search", "ace/edit_session", "ace/editor", "ace/config"], function(a, b) {
	function c(a, b, c) {
	...
	function e(a) {
		a.$multiselectOnSessionChange || (a.$onAddRange = a.$onAddRange.bind(a), a.$onRemoveRange = a.$onRemoveRange.bind(a), a.$onMultiSelect = a.$onMultiSelect.bind(a), a.$onSingleSelect = a.$onSingleSelect.bind(a), a.$multiselectOnSessionChange = b.onSessionChange.bind(a), a.$multiselectOnSessionChange(a), a.on("changeSession", a.$multiselectOnSessionChange), a.on("mousedown", j), a.commands.addCommands(m.defaultCommands), f(a))
	}
	...
	var g = a("./range_list").RangeList,
		h = a("./range").Range,
		i = a("./selection").Selection,
		j = a("./mouse/multi_select_handler").onMouseDown,
		k = a("./lib/event"),
		l = a("./lib/lang"),
		m = a("./commands/multi_select_commands");
	b.commands = m.defaultCommands.concat(m.multiSelectCommands);
	var n = a("./search").Search,
		o = new n,
		p = a("./edit_session").EditSession;
	(function() {
		...
		this.ranges = null, this.rangeList = null, this.addRange = function(a, b) {
			...
			if (this.rangeCount > 1) {
				var a = this.rangeList.ranges,
					b = a[a.length - 1],
					c = h.fromPoints(a[0].start, b.end);
	...
	(function() {
		this.updateSelectionMarkers = function() {
			...
			if (!this.inVirtualSelectionMode) {
				...
				for (var k = g.ranges.length; k--;) {
					if (c)
						for (; k > 0 && g.ranges[k].start.row == g.ranges[k - 1].end.row;) k--;
					j.fromOrientedRange(g.ranges[k]), j.id = g.ranges[k].marker, this.selection = e.selection = j;
					var l = a.exec(this, b || {});
					void 0 !== d && (d = l), j.toOrientedRange(g.ranges[k])
				}
				j.detach(), this.selection = e.selection = f, this.inVirtualSelectionMode = !1, f._eventRegistry = h, f.mergeOverlappingRanges();
				var m = this.renderer.$scrollAnimation;
				return this.onCursorChange(), this.onSelectionChange(), m && m.from == m.to && this.renderer.animateScrolling(m.from), d
			}
			...
			if (!this.$readOnly) {
				...
				if (this._signal("paste", b), a = b.text, !this.inMultiSelectMode || this.inVirtualSelectionMode) return this.insert(a);
				var c = a.split(/\r\n|\r|\n/),
			...
			else var i = this.session.documentToScreenPosition(d ? c.end : c.start),
				g = this.session.screenToDocumentPosition(i.row + a, i.column); if (d) {
			...
			var d = this.session,
				e = d.multiSelect,
				f = e.toOrientedRange();
			f.isEmpty() && (f = d.getWordRange(f.start.row, f.start.column), f.cursor = -1 == a ? f.start : f.end, this.multiSelect.addRange(f));
			var g = d.getTextRange(f),
				h = c(d, g, a);
			h && (h.cursor = -1 == a ? h.start : h.end, this.$blockScrolling += 1, this.session.unfold(h), this.multiSelect.addRange(h), this.$blockScrolling -= 1, this.renderer.scrollCursorIntoView(null, .5)), b && this.multiSelect.substractPoint(f.cursor)
		...
		var c = a.oldSession;
		c && (c.multiSelect.removeEventListener("addRange", this.$onAddRange), c.multiSelect.removeEventListener("removeRange", this.$onRemoveRange), c.multiSelect.removeEventListener("multiSelect", this.$onMultiSelect), c.multiSelect.removeEventListener("singleSelect", this.$onSingleSelect)), b.multiSelect.on("addRange", this.$onAddRange), b.multiSelect.on("removeRange", this.$onRemoveRange), b.multiSelect.on("multiSelect", this.$onMultiSelect), b.multiSelect.on("singleSelect", this.$onSingleSelect), this.inMultiSelectMode != b.selection.inMultiSelectMode && (b.selection.inMultiSelectMode ? this.$onMultiSelect() : this.$onSingleSelect())
	...
	var c = a("./lib/oop"),
		...
		r = function(a, b) {
			...
			this.container = c, this.renderer = a, this.commands = new o(f.isMac ? "mac" : "win", p), this.textInput = new g(a.getTextAreaContainer(), this), this.renderer.textarea = this.textInput.getElement(), this.keyBinding = new j(this), this.$mouseHandler = new h(this), new i(this), this.$blockScrolling = 0, this.$search = (new l).set({
				wrap: !0
			}), this.$historyTracker = this.$historyTracker.bind(this), this.commands.on("exec", this.$historyTracker), this.$initOperationListeners(), this._$emitInputEvent = e.delayedCall(function() {
	...
	(function() {
		c.implement(this, n), this.$initOperationListeners = function() {
			...
			this.selections = [], this.commands.on("exec", function(b) {
				this.startOperation(b);
				var c = b.command;
				if ("fileJump" == c.aceCommandGroup) {
					var d = this.prevOp;
					d && "fileJump" == d.command.aceCommandGroup || (this.lastFileJumpPos = a(this.selections))
				} else this.lastFileJumpPos = null
			}.bind(this), !0), this.commands.on("afterExec", function(a) {
				var b = a.command;
				"fileJump" == b.aceCommandGroup && this.lastFileJumpPos && !this.curOp.selectionChanged && this.selection.fromJSON(this.lastFileJumpPos), this.endOperation(a)
			}.bind(this), !0), this.$opResetTimer = e.delayedCall(this.endOperation.bind(this)), this.on("change", function() {
			...
			b && b.scrollIntoView && this.$blockScrolling++, this.selections.push(this.selection.toJSON())
		}, this.endOperation = function() {
			if (this.curOp) {
				var a = this.curOp.command;
				if (a && a.scrollIntoView) {
					switch (this.$blockScrolling--, a.scrollIntoView) {
						case "center":
						...
						case "selectionPart":
							...
							(b.start.row >= c.lastRow || b.end.row <= c.firstRow) && this.renderer.scrollSelectionIntoView(this.selection.anchor, this.selection.lead)
					}
					"animate" == a.scrollIntoView && this.renderer.animateScrolling(this.curOp.scrollTop)
				}
			...
			if (a)
				if ("string" == typeof a) {
					this.$keybindingId = a;
					var b = this;
					q.loadModule(["keybinding", a], function(c) {
						b.$keybindingId == a && b.keyBinding.setKeyboardHandler(c && c.handler)
					})
				} else this.$keybindingId = null, this.keyBinding.setKeyboardHandler(a);
			...
			if (this.session.$bracketHighlight && (this.session.removeMarker(this.session.$bracketHighlight), this.session.$bracketHighlight = null), !this.$highlightPending) {
				...
				this.$highlightPending = !0, setTimeout(function() {
					...
					if (b) var c = new m(b.row, b.column, b.row, b.column + 1);
					else if (a.session.$mode.getMatching) var c = a.session.$mode.getMatching(a.session);
					c && (a.session.$bracketHighlight = a.session.addMarker(c, "ace_bracket", "text"))
				}, 50)
			}
		}, this.focus = function() {
			var a = this;
			...
			var b, c = a.data,
				d = c.range;
			b = d.start.row == d.end.row && "insertLines" != c.action && "removeLines" != c.action ? d.end.row : 1 / 0, this.renderer.updateLines(d.start.row, b), this._signal("change", a), this.$cursorChange()
			...
			var a = this.session;
			if (a.$selectionMarker && a.removeMarker(a.$selectionMarker), a.$selectionMarker = null, this.selection.isEmpty()) this.$updateHighlightActiveLine();
			else {
				var b = this.selection.getRange(),
			...
			if (!this.$readOnly) {
				...
				this._signal("paste", b), this.insert(b.text, !0)
			}
		}, this.execCommand = function(a, b) {
			this.commands.exec(a, this, b)
		}, this.insert = function(a, b) {
			var c = this.session,
				d = c.getMode(),
			...
			if (this.session.getUseSoftTabs()) var j = h - i % h,
				...
				for (var j = i % h;
					" " == f[b.start.column] && j;) b.start.column--, j--;
			...
			if (!b.inMultiSelectMode || this.inVirtualSelectionMode) {
				var c = b.toOrientedRange(),
					d = this.$getSelectedRows(c),
					e = a.call(this, d.first, d.last);
				c.moveBy(e, 0), b.fromOrientedRange(c)
			} else {
				var f = b.rangeList.ranges;
				b.rangeList.detach(this.session);
				for (var g = f.length; g--;) {
					for (var h = g, d = f[g].collapseRows(), i = d.end.row, j = d.start.row; g-- && (d = f[g].collapseRows(), j - d.end.row <= 1);) j = d.end.row;
					g++;
					for (var e = a.call(this, j, i); h >= g;) f[h].moveBy(e, 0), h--
				}
				b.fromOrientedRange(b.ranges[0]), b.rangeList.attach(this.session)
			}
			...
			d = c && c.cursor || d, d && (a ? c && c.isEqual(this.getSelectionRange()) ? this.clearSelection() : this.selection.selectTo(d.row, d.column) : this.selection.moveTo(d.row, d.column))
			...
			b && (b.setSmoothBlinking("smooth" == a), b.isBlinking = !this.$readOnly && "wide" != a)
		}
	}).call(r.prototype), q.defineOptions(r.prototype, "editor", {
		selectionStyle: {
			set: function(a) {
		...
		readOnly: {
			set: function(a) {
				this.textInput.setReadOnly(a), this.$resetCursorStyle()
			},
			initialValue: !1
		},
		cursorStyle: {
...
}), ace.define("ace/mouse/multi_select_handler", ["require", "exports", "module", "ace/lib/event"], function(a, b) {
	function c(a, b) {
		return a.row == b.row && a.column == b.column
	}
	...
	function d(a) {
		var b = a.domEvent,
			...
			g = a.getAccelKey(),
			h = a.getButton();
		if (a.editor.inMultiSelectMode && 2 == h) return void a.editor.textInput.onContextMenu(a.domEvent);
		if (!g && !d) return void(0 === h && a.editor.inMultiSelectMode && a.editor.exitMultiSelectMode());
		var i = a.editor,
			j = i.selection,
			k = i.inMultiSelectMode,
			l = a.getDocumentPosition(),
			m = j.getCursor(),
			n = a.inSelection() || j.isEmpty() && c(l, m),
			o = a.x,
			p = a.y,
			q = function(a) {
			...
			r = function() {
				...
				c(u, a) && c(b, j.selectionLead) || (u = a, i.selection.moveToPosition(b), i.renderer.scrollCursorIntoView(), i.removeSelectionMarkers(v), v = j.rectangularRangeBlock(u, t), v.forEach(i.addSelectionMarker, i), i.updateSelectionMarkers())
			},
			s = i.session,
		...
		if (!g || d || f || 0 !== h) {
			if (d && 0 === h) {
				a.stop(), k && !g ? j.toSingleRange() : !k && g && j.addRange();
				var v = [];
				f ? (t = s.documentToScreenPosition(j.lead), r()) : j.moveToPosition(l);
				var w = function() {
						clearInterval(y), i.removeSelectionMarkers(v);
						for (var a = 0; a < v.length; a++) j.addRange(v[a])
					},
					x = r;
				e.capture(i.container, q, w);
			...
			if (!k && n) return;
			if (!k) {
				var z = j.toOrientedRange();
				i.addSelectionMarker(z)
			}
			var A = j.rangeList.rangeAtPoint(l);
			i.$blockScrolling++, i.once("mouseup", function() {
				var a = j.toOrientedRange();
				A && a.isEmpty() && c(A.cursor, a.cursor) ? j.substractPoint(a.cursor) : (z && (i.removeSelectionMarker(z), j.addRange(z)), j.addRange(a)), i.$blockScrolling--
			})
		}
	}
	var e = a("../lib/event");
	b.onMouseDown = d
}), ace.define("ace/lib/useragent", ["require", "exports", "module"], function(a, b) {
	if (b.OS = {
		LINUX: "LINUX",
		MAC: "MAC",
		WINDOWS: "WINDOWS"
		...
		var c = (navigator.platform.match(/mac|win|linux/i) || ["other"])[0].toLowerCase(),
...
}), ace.define("ace/commands/multi_select_commands", ["require", "exports", "module", "ace/keyboard/hash_handler"], function(a, b) {
	b.defaultCommands = [{
		name: "addCursorAbove",
		exec: function(a) {
			a.selectMoreLines(-1)
...
}), ace.define("ace/lib/oop", ["require", "exports", "module"], function(a, b) {
...
}), ace.define("ace/worker/worker_client", ["require", "exports", "module", "ace/lib/oop", "ace/lib/net", "ace/lib/event_emitter", "ace/config"], function(a, b) {
	var c = a("../lib/oop"),
		d = a("../lib/net"),
		e = a("../lib/event_emitter").EventEmitter,
		f = a("../config"),
		g = function(b, c, d, e) {
			if (this.$sendDeltaQueue = this.$sendDeltaQueue.bind(this), this.changeListener = this.changeListener.bind(this), this.onMessage = this.onMessage.bind(this), a.nameToUrl && !a.toUrl && (a.toUrl = a.nameToUrl), f.get("packaged") || !a.toUrl) e = e || f.moduleUrl(c, "worker");
			else {
	...
	(function() {
		c.implement(this, e), this.onMessage = function(a) {
			...
			return location.host ? (a = a.replace(/^[a-z]+:\/\/[^\/]+/, ""), a = location.protocol + "//" + location.host + ("/" == a.charAt(0) ? "" : location.pathname.replace(/\/[^\/]*$/, "")) + "/" + a.replace(/^[\/]+/, "")) : a
			...
			a && (this.deltaQueue = null, a.length > 20 && a.length > this.$doc.getLength() >> 1 ? this.call("setValue", [this.$doc.getValue()]) : this.emit("change", {
				data: a
			}))
...
}), ace.define("ace/lib/keys", ["require", "exports", "module", "ace/lib/oop"], function(a, b) {
	var c = a("./oop"),
		d = function() {
			var a, b, d = {
				MODIFIER_KEYS: {
			...
			return c.mixin(d, d.MODIFIER_KEYS), c.mixin(d, d.PRINTABLE_KEYS), c.mixin(d, d.FUNCTION_KEYS), d.enter = d["return"], d.escape = d.esc, d.del = d["delete"], d[173] = "-",
				function() {
	...
	c.mixin(b, d), b.keyCodeToString = function(a) {
		return (d[a] || String.fromCharCode(a)).toLowerCase()
	}
}), ace.define("ace/placeholder", ["require", "exports", "module", "ace/range", "ace/lib/event_emitter", "ace/lib/oop"], function(a, b) {
	var c = a("./range").Range,
		d = a("./lib/event_emitter").EventEmitter,
		e = a("./lib/oop"),
	...
	(function() {
		e.implement(this, d), this.setup = function() {
			var a = this,
				...
				d = this.session,
				e = this.$pos;
			this.pos = b.createAnchor(e.row, e.column), this.markerId = d.addMarker(new c(e.row, e.column, e.row, e.column + this.length), this.mainClass, null, !1), this.pos.on("change", function(b) {
...
}), ace.define("ace/lib/event", ["require", "exports", "module", "ace/lib/keys", "ace/lib/useragent"], function(a, b) {
	function c(a, b, c) {
		var i = f(b);
		if (!e.isMac && g) {
			if ((g[91] || g[92]) && (i |= 8), g.altGr) {
				if (3 == (3 & i)) return;
				g.altGr = 0
			}
			if (18 === c || 17 === c) {
				var j = b.location || b.keyLocation;
				if (17 === c && 1 === j) h = b.timeStamp;
				else if (18 === c && 3 === i && 2 === j) {
					var k = -h;
					h = b.timeStamp, k += h, 3 > k && (g.altGr = !0)
				}
			}
		}
		...
		return 8 & i && (91 === c || 93 === c) && (c = -1), i || 13 !== c || !b.location && 3 !== b.keyLocation || (a(b, i, -c), !b.defaultPrevented) ? i || c in d.FUNCTION_KEYS || c in d.PRINTABLE_KEYS ? a(b, i, c) : !1 : void 0
	}
	var d = a("./keys"),
		e = a("./useragent");
	b.addListener = function(a, b, c) {
		...
		return "dblclick" == a.type ? 0 : "contextmenu" == a.type || a.ctrlKey && e.isMac ? 2 : a.preventDefault ? a.button : {
			1: 0,
			2: 2,
			4: 1
		}[a.button]
		...
		b.addListener(a, "mousedown", function(a) {
			...
			if (d[f]("mousedown", a), j > 4) j = 0;
			else if (j > 1) return d[f](k[j], a)
...
}), ace.define("ace/mode/folding/fold_mode", ["require", "exports", "module", "ace/range"], function(a, b) {
	var c = a("../../range").Range,
		d = b.FoldMode = function() {};
	(function() {
		this.foldingStartMarker = null, this.foldingStopMarker = null, this.getFoldWidget = function(a, b, c) {
			...
			var f = {
				...
				g = a.$findOpeningBracket(b, f);
			return g ? (g.column++, f.column--, c.fromPoints(g, f)) : void 0
		}
	}).call(d.prototype)
}), ace.define("ace/lib/dom", ["require", "exports", "module"], function(a, b) {
	if ("undefined" != typeof document) {
		...
		}, void 0 !== window.pageYOffset ? (b.getPageScrollTop = function() {
...
}), ace.define("ace/theme/textmate", ["require", "exports", "module", "ace/lib/dom"], function(a, b) {
		...
		if (function() {
			...
			if (a === c) f && this.push.apply(this, e);
				...
				var g = Math.min(b, c - a),
					...
					k = c - g;
				if (h > i)
					for (var m = 0; j > m; ++m) this[i + m] = this[h + m];
				else if (i > h)
					for (m = j; m--;) this[i + m] = this[h + m];
				if (f && a === k) this.length = k, this.push.apply(this, e);
				else
					for (this.length = k + f, m = 0; f > m; ++m) this[a + m] = e[m]
			}
			return d
		};
	...
	if (Array.prototype.forEach || (Array.prototype.forEach = function(a) {
		...
		var b = q && "[object String]" == m(this) ? this.split("") : H(this),
			d = b.length >>> 0;
		if (!d) return -1;
		var e = 0;
		for (arguments.length > 1 && (e = c(arguments[1])), e = e >= 0 ? e : Math.max(0, d + e); d > e; e++)
		...
		if (!d) return -1;
		var e = d - 1;
		for (arguments.length > 1 && (e = Math.min(e, c(arguments[1]))), e = e >= 0 ? e : d - Math.abs(e); e >= 0; e--)
	...
	if (Object.defineProperty) {
		var t = b({}),
			u = "undefined" == typeof document || b(document.createElement("div"));
	...
	e && d || (RegExp.prototype.exec = function(f) {
		...
		if ("string" == typeof f && i) {
			if (!d && i.length > 1 && b(i, "") > -1 && (h = RegExp(this.source, c.replace.call(a(this), "g", "")), c.replace.call(f.slice(i.index), h, function() {
				...
				for (var j = 1; j < i.length; j++) g = this._xregexp.captureNames[j - 1], g && (i[g] = i[j]);
...
}), ace.define("ace/ext/error_marker", ["require", "exports", "module", "ace/line_widgets", "ace/lib/dom", "ace/range"], function(a, b) {
	function c(a, b, c) {
	...
	function d(a, b, d) {
		...
		if (e.length) {
			...
			0 > f && (f = -f - 1), f >= e.length - 1 ? f = d > 0 ? 0 : e.length - 1 : 0 === f && 0 > d && (f = e.length - 1);
			var h = e[f];
			if (h && d) {
				if (h.row === b) {
	...
	b.showErrorMarker = function(a, b) {
		...
		var m = {
			...
			n = m.el.appendChild(f.createElement("div")),
			o = m.el.appendChild(f.createElement("div"));
...
}), ace.define("ace/lib/fixoldbrowsers", ["require", "exports", "module", "ace/lib/regexp", "ace/lib/es5-shim"], function(a) {
	a("./regexp"), a("./es5-shim")
}), ace.define("ace/line_widgets", ["require", "exports", "module", "ace/lib/oop", "ace/lib/dom", "ace/range"], function(a, b) {
	function c(a) {
		this.session = a, this.session.widgetManager = this, this.session.getRowLength = this.getRowLength, this.session.$getWidgetScreenLength = this.$getWidgetScreenLength, this.updateOnChange = this.updateOnChange.bind(this), this.renderWidgets = this.renderWidgets.bind(this), this.measureWidgets = this.measureWidgets.bind(this), this.session._changedWidgets = [], this.detach = this.detach.bind(this), this.session.on("change", this.updateOnChange)
	} {
		var d = (a("./lib/oop"), a("./lib/dom"));
		a("./range").Range
	}(function() {
		this.getRowLength = function(a) {
			...
			a.widgetManager && a.widgetManager != this && a.widgetManager.detach(), this.editor != a && (this.detach(), this.editor = a, this.editor.on("changeSession", this.detach), a.widgetManager = this, a.setOption("enableLineWidgets", !0), a.renderer.on("beforeRender", this.measureWidgets), a.renderer.on("afterRender", this.renderWidgets))
		}, this.detach = function(a) {
			if (!a || a.session != this.session) {
				var b = this.editor;
				if (b) {
					b.off("changeSession", this.detach), this.editor = null, b.widgetManager = null, b.renderer.off("beforeRender", this.measureWidgets), b.renderer.off("afterRender", this.renderWidgets);
			...
			if (b) {
				var c = a.data,
					...
					f = d.end.row - e;
				if (0 === f);
				else if ("removeText" == c.action || "removeLines" == c.action) {
			...
			this.session.lineWidgets && (this.session.lineWidgets[a.row] = void 0), this.session._emit("changeFold", {
				data: {
					start: {
						row: a.row
					}
			...
			if (c && c.length) {
				...
				1 / 0 != e && (this.session._emit("changeFold", {
					data: {
						start: {
							row: e
						}
...
}), ace.define("ace/document", ["require", "exports", "module", "ace/lib/oop", "ace/lib/event_emitter", "ace/range", "ace/anchor"], function(a, b) {
	var c = a("./lib/oop"),
		d = a("./lib/event_emitter").EventEmitter,
		e = a("./range").Range,
		f = a("./anchor").Anchor,
	...
	(function() {
		c.implement(this, d), this.setValue = function(a) {
			var b = this.getLength();
			this.remove(new e(0, 0, b, this.getLine(b - 1).length)), this.insert({
				row: 0,
				column: 0
			}, a)
		}, this.getValue = function() {
			...
			var c = this.$split(b),
				d = c.splice(0, 1)[0],
				e = 0 == c.length ? null : c.splice(c.length - 1, 1)[0];
			...
			for (; b.length > 61440;) {
				var c = this._insertLines(a, b.slice(0, 61440));
				b = b.slice(61440), a = c.row
			}
			var d = [a, 0];
			d.push.apply(d, b), this.$lines.splice.apply(this.$lines, d);
			var f = new e(a, 0, a + b.length, 0),
				g = {
					action: "insertLines",
					range: f,
					lines: b
			...
			if (b != c) {
				var d = new e(a, b, a, c),
					f = this.getLine(a),
					g = f.substring(b, c),
					h = f.substring(0, b) + f.substring(c, f.length);
			...
			if (this.remove(a), b) var c = this.insert(a.start, b);
			else c = a.start;
			return c
		}, this.applyDeltas = function(a) {
			for (var b = 0; b < a.length; b++) {
				var c = a[b],
					d = e.fromPoints(c.range.start, c.range.end);
...
HookCollection.prototype = {
	chain: function(a, b) {
		var c = this[a];
		if (!c) throw new Error("unknown hook " + a);
		this[a] = c === identity ? b : function() {
			var a = Array.prototype.slice.call(arguments, 0);
			return a[0] = c.apply(null, a), b.apply(null, a)
		}
	},
	set: function(a, b) {
...
function() {
	...
	function b(a, b) {
		this.buttonBar = k.getElementById("wmd-button-bar" + a), this.preview = k.getElementById("wmd-preview" + a), this.input = b
	}
	...
	function c(b) {
		...
		this.init = function() {
			...
			var i = "undefined" != typeof require ? require("ace/range").Range : ace.require("ace/range").Range,
				j = function(a, b) {
	...
	function e(a, b, d, e, f, g, h, i) {
		function j(a) {
			if (0 !== a.length) {
				var b = a.pop();
				s.$markdownCommands.addCommand({
					name: h(b),
					bindKey: q[b],
					exec: function() {
						return l(t[b]), !0
		...
		function l(a) {
			...
			if (a.textOp) {
				...
				var h = g.getChunks(),
					i = function() {
						h && g.setChunks(h), g.restore(), e.refresh()
					},
					j = a.textOp(h, i);
				j || i()
			}
		...
		function m(a, c) {
			var d = "0px",
				e = "-20px",
				f = "-40px",
				g = a.getElementsByTagName("span")[0];
			...
			}), a.className = a.className.replace(/ disabled/g, "")) : (g.style.backgroundPosition = a.XShift + " " + e, a.onmouseover = a.onmouseout = a.onclick = function() {}, a.className += " disabled")
		}

		function n(a) {
			return "string" == typeof a && (a = f[a]),
				function() {
					a.apply(f, arguments)
				}
		}
		...
		function p() {
			...
			var e = 0,
				f = function(b, c, f, g) {
					...
					return h.id = b + a, h.appendChild(i), h.title = c, h.XShift = f, g && (h.textOp = g), m(h, !0), d.appendChild(h), h
				},
				i = function(b) {
					var c = document.createElement("li");
					c.className = "wmd-spacer wmd-spacer" + b, c.id = "wmd-spacer" + b + a, d.appendChild(c), e += 25
				};
			if (t.bold = f("wmd-bold-button", r("bold"), "0px", n("doBold")), t.italic = f("wmd-italic-button", r("italic"), "-20px", n("doItalic")), i(1), t.link = f("wmd-link-button", r("link"), "-40px", n(function(a, b) {
				return this.doLinkOrImage(a, b, !1)
			})), t.quote = f("wmd-quote-button", r("quote"), "-60px", n("doBlockquote")), t.code = f("wmd-code-button", r("code"), "-80px", n("doCode")), t.image = f("wmd-image-button", r("image"), "-100px", n(function(a, b) {
				return this.doLinkOrImage(a, b, !0)
			})), i(2), t.olist = f("wmd-olist-button", r("olist"), "-120px", n(function(a, b) {
				this.doList(a, b, !0)
			})), t.ulist = f("wmd-ulist-button", r("ulist"), "-140px", n(function(a, b) {
				this.doList(a, b, !1)
			})), t.heading = f("wmd-heading-button", r("heading"), "-160px", n("doHeading")), t.hr = f("wmd-hr-button", r("hr"), "-180px", n("doHorizontalRule")), i(3), t.undo = f("wmd-undo-button", r("undo"), "-200px", null), t.undo.execute = function() {
				s.session.getUndoManager().undo()
			}, t.redo = f("wmd-redo-button", r("redo"), "-220px", null), t.redo.execute = function() {
				s.session.getUndoManager().redo()
			}, t.table = f("wmd-table-button", r("table"), "-240px", n("doTable")), g) {
				var j = document.createElement("li"),
					k = document.createElement("span");
				j.appendChild(k), j.className = "wmd-button wmd-help-button", j.id = "wmd-help-button" + a, j.XShift = "-240px", j.isHelp = !0, j.style.right = "0px", j.title = h("help"), j.onclick = g.handler, m(j, !0), d.appendChild(j), t.help = j
			}
			u.setUndoRedoButtonStates(), s.session.on("change", function() {
				u.setUndoRedoButtonStates()
			})
		}
		var r = function(a) {
				return h(a) + " " + i(a)
		...
		this.setUndoRedoButtonStates = function() {
			setTimeout(function() {
				m(t.undo, s.session.getUndoManager().hasUndo()), m(t.redo, s.session.getUndoManager().hasRedo())
			}, 50)
		};
		var u = this;
		p();
		var v = "keydown";
		o.isOpera && (v = "keypress"), s.$markdownCommands = new(ace.require("ace/keyboard/hash_handler").HashHandler), s.$markdownCommands.$id = "ace/keyboard/markdown", j(["bold", "italic", "link", "quote", "code", "image", "olist", "ulist", "heading", "hr", "table"]), this.buttons = t, this.setButtonState = m
	}
	...
	Markdown.Editor = function(a, c, g) {
		if (g = g || {}, "function" == typeof g.handler && (g = {
			helpButton: g
		}), g.strings = g.strings || {}, g.helpButton && (g.strings.help = g.strings.help || g.helpButton.title), g.keyStrokes)
			for (var h in g.keyStrokes) q[h] = g.keyStrokes[h];
		var i = function(a) {
		...
		this.run = function(h, n) {
			if (!k) {
				k = new b(c, h);
				var o, p = new f(j, i),
					r = new d(a, k, function() {
						j.onPreviewRefresh()
					}, n),
					s = "undefined" != typeof require ? require("ace/lib/useragent") : ace.require("ace/lib/useragent"),
					t = function(a) {
						var b = q[a][s.isMac ? "mac" : "win"],
							c = b.indexOf("|");
		...
		setTimeout(function() {
			j();
			var a = b.length;
			if (void 0 !== e.selectionStart) e.selectionStart = 0, e.selectionEnd = a;
	...
	t.prefixes = "(?:\\s{4,}|\\s*>|\\s*-\\s+|\\s*\\d+\\.|=|\\+|-|_|\\*|#|\\s*\\[[^\n]]+\\]:)", t.unwrap = function(a) {
		...
		if (g >= c && (2 != g || 1 != c)) a.before = a.before.replace(l("[*]{" + c + "}$", ""), ""), a.after = a.after.replace(l("^[*]{" + c + "}", ""), "");
		else if (!a.selection && f) {
		...
		if (!(a.endTag.length > 1 && a.startTag.length > 0)) {
			...
			var e = this,
				f = function(f) {
					d.parentNode.removeChild(d), null !== f && (a.selection = (" " + a.selection).replace(/([^\\](?:\\\\)*)(?=[[\]])/g, "$1\\").substr(1), a.startTag = c ? "![" : "[", a.endTag = "](" + g(f) + ")", a.selection || (a.selection = e.getString(c ? "imagedescription" : "linkdescription"))), b()
				};
		...
		if (a.selection = a.selection.replace(/\s+/g, " "), a.selection = a.selection.replace(/(^\s+|\s+$)/g, ""), a.selection || (a.before.indexOf("\n") < 0 ? (a.selection = a.before, a.before = "") : a.before = a.before.replace(/^((?:.|\n)*)\n(.+)$/, function(b, c, d) {
			return a.selection = d, c + "\n"
		}), a.after = a.after.replace(/^([^\n]+)/, function(b, c) {
			return a.selection = a.selection + c, ""
		})), !a.selection) return a.startTag = "## ", a.selection = this.getString("headingexample"), void(a.endTag = "");
		var b = 0;
		return a.findTags(/#+[ ]*/, /[ ]*#+/), console.log("chunk", a.startTag, a.selection, a.endTag), void(/#*/.test(a.startTag) && (b = l.lastMatch.length, 0 == b && (a.startTag = " "), 4 > b ? a.startTag = "#" + a.startTag : (a.findTags(/#+[ ]*/, /[ ]*#+/), a.startTag = ""), a.endTag = "", a.startTag = a.startTag + a.selection, a.selection = ""))
	}, t.doHorizontalRule = function(a) {
		a.startTag = "----------\n", a.selection = "", a.skipLines(2, 1, !0)
	}, t.doTable = function(a) {
		a.endTag = "", a.startTag = "\n| Col1      |     Col2 |   Col3   |\n| :-------- | --------:| :------: |\n| field1    |   field2 |  field3  |\n", a.selection = "", a.skipLines(1, 1, !0)
	}
}(), ace.define("ace/keyboard/emacs", ["require", "exports", "module", "ace/lib/dom", "ace/incremental_search", "ace/commands/incremental_search_commands", "ace/keyboard/hash_handler", "ace/lib/keys"], function(a, b) {
	var c = a("../lib/dom");
	a("../incremental_search");
	var d = a("../commands/incremental_search_commands"),
		e = function(a, b) {
			var c = this.scroller.getBoundingClientRect(),
				d = Math.floor((a + this.scrollLeft - c.left - this.$padding) / this.characterWidth),
				e = Math.floor((b + this.scrollTop - c.top) / this.lineHeight);
	...
	n.forEach(function(a) {
		...
		c.$handlesEmacsOnCopy || (c.$handlesEmacsOnCopy = !0, b.handler.commands.killRingSave.exec(c), delete c.$handlesEmacsOnCopy)
		...
		var e = a.editor;
		if (-1 == b && (e.pushEmacsMark(), a.count)) {
		...
		if ("\x00" == c) return void 0;
		var g = m[b];
		if ("c-" == g || a.universalArgument) {
			var h = String(a.count || 0),
				i = parseInt(c[c.length - 1]);
			if ("number" == typeof i && !isNaN(i)) return a.count = parseInt(h + i), {
				command: "null"
			};
			a.universalArgument && (a.count = 4)
		}
		a.universalArgument = !1, g && (c = g + c), a.keyChain && (c = a.keyChain += " " + c);
		var j = this.commandKeyBinding[c];
		if (a.keyChain = "null" == j ? c : "", !j) return void 0;
		if ("null" === j) return {
			command: "null"
		};
		if ("universalArgument" === j) return a.universalArgument = !0, {
			command: "null"
		};
		var k;
		if ("string" != typeof j && (k = j.args, j.command && (j = j.command), "goorselect" === j && (j = e.emacsMark() ? k[1] : k[0], k = null)), "string" == typeof j && (("insertstring" === j || "splitline" === j || "togglecomment" === j) && e.pushEmacsMark(), j = this.commands[j] || e.commands.commands[j], !j)) return void 0;
		if (j.readonly || j.isYank || (a.lastCommand = null), a.count) {
			var i = a.count;
			if (a.count = 0, !j || !j.handlesCount) return {
				args: k,
				command: {
					exec: function(a, b) {
						for (var c = 0; i > c; c++) j.exec(a, b)
					}
				}
			};
			k || (k = {}), "object" == typeof k && (k.count = i)
		}
		return {
			command: j,
			args: k
		...
		setMark: {
			exec: function(a, b) {
				if (b && b.count) {
					var c = a.popEmacsMark();
					return void(c && a.selection.moveCursorToPosition(c))
				}
				var c = a.emacsMark(),
					d = !0;
				if (d && (c || !a.selection.isEmpty())) return a.pushEmacsMark(), void a.clearSelection();
				if (c) {
					var e = a.getCursorPosition();
					if (a.selection.isEmpty() && c.row == e.row && c.column == e.column) return void a.pushEmacsMark()
				}
				c = a.getCursorPosition(), a.setEmacsMark(c), a.selection.setSelectionAnchor(c.row, c.column)
			},
			readonly: !0,
			handlesCount: !0,
			multiSelectAction: "forEach"
		...
		exchangePointAndMark: {
			exec: function(a, b) {
				...
				if (b.count) {
					...
					return c.clearSelection(), c.moveCursorToPosition(a.popEmacsMark()), void a.pushEmacsMark(d)
				}
				var e = a.getLastEmacsMark(),
					f = c.getRange();
				return f.isEmpty() ? void c.selectToPosition(e) : void c.setSelectionRange(f, !c.isBackwards())
			},
			readonly: !0,
			handlesCount: !0,
			multiSelectAction: "forEach"
		...
		yankRotate: function(a) {
			"yank" == a.keyBinding.$data.lastCommand && (a.undo(), a.onPaste(b.killRing.rotate()), a.keyBinding.$data.lastCommand = "yank")
		},
		killRegion: {
			exec: function(a) {
		...
		killRingSave: {
			exec: function(a) {
				b.killRing.add(a.getCopyText()), setTimeout(function() {
					var b = a.selection,
						c = b.getRange();
					a.pushEmacsMark(b.isBackwards() ? c.end : c.start), b.clearSelection()
				}, 0)
			},
			readonly: !0
		},
		keyboardQuit: function(a) {
			a.selection.clearSelection(), a.setEmacsMark(null)
		},
...
}), ace.define("ace/incremental_search", ["require", "exports", "module", "ace/lib/oop", "ace/range", "ace/search", "ace/search_highlight", "ace/commands/incremental_search_commands", "ace/lib/dom", "ace/commands/command_manager", "ace/editor", "ace/config"], function(a, b) {
	function c() {
		this.$options = {
			wrap: !1,
			skipCurrent: !1
		}, this.$keyboardHandler = new i(this)
	}
	var d = a("./lib/oop"),
		e = a("./range").Range,
		f = a("./search").Search,
		g = a("./search_highlight").SearchHighlight,
		h = a("./commands/incremental_search_commands"),
	...
	function() {
		this.activate = function(a, b) {
			...
			var b = this.$editor.session,
				c = b.$isearchHighlight = b.$isearchHighlight || b.addDynamicMarker(new g(null, "ace_isearch-result", "text"));
			c.setRegexp(a), b._emit("changeBackMarker")
		}, this.cancelSearch = function(a) {
			var b = this.$editor;
			return this.$prevNeedle = this.$options.needle, this.$options.needle = "", a ? (b.moveCursorToPosition(this.$startPos), this.$currentPos = this.$startPos) : b.pushEmacsMark && b.pushEmacsMark(this.$startPos, !1), this.highlight(null), e.fromPoints(this.$currentPos, this.$currentPos)
			...
			var d = this.$editor.session,
				f = this.find(d);
			return f && (c.backwards && (f = e.fromPoints(f.end, f.start)), this.$editor.moveCursorToPosition(f.end), a && (this.$currentPos = f.end), this.highlight(c.re)), this.statusMessage(f), f
			...
			return this.highlightAndFindWithNeedle(!1, function(a) {
				return a.length > 0 ? a.substring(0, a.length - 1) : a
			})
...
}), ace.define("ace/commands/incremental_search_commands", ["require", "exports", "module", "ace/config", "ace/lib/oop", "ace/keyboard/hash_handler", "ace/commands/occur_commands"], function(a, b) {
	function c(a) {
		this.$iSearch = a
	}
	var d = a("../config"),
		e = a("../lib/oop"),
		f = a("../keyboard/hash_handler").HashHandler,
		g = a("./occur_commands").occurStartCommand;
	b.iSearchStartCommands = [{
		...
		exec: function(a) {
			var b = a.$editor,
				c = b.selection.getRangeOfMovements(function(a) {
...
}), ace.define("ace/commands/occur_commands", ["require", "exports", "module", "ace/config", "ace/occur", "ace/keyboard/hash_handler", "ace/lib/oop"], function(a, b) {
	function c() {}
	var d = (a("../config"), a("../occur").Occur),
		e = {
			name: "occur",
			exec: function(a, b) {
				var e = !!a.session.$occur,
					f = (new d).enter(a, b);
				f && !e && c.installIn(a)
			},
			readOnly: !0
		...
		f = [{
			...
			exec: function(a) {
				...
				b && (b.exit(a, {
					translatePosition: !0
				}), a.session.$occur || c.uninstallFrom(a))
			},
			readOnly: !0
		}],
		g = a("../keyboard/hash_handler").HashHandler,
		h = a("../lib/oop");
	h.inherits(c, g),
	function() {
...
}), ace.define("ace/occur", ["require", "exports", "module", "ace/lib/oop", "ace/range", "ace/search", "ace/edit_session", "ace/search_highlight", "ace/lib/dom"], function(a, b) {
	function c() {}
	var d = a("./lib/oop"),
		e = (a("./range").Range, a("./search").Search),
		f = a("./edit_session").EditSession,
		g = a("./search_highlight").SearchHighlight;
	d.inherits(c, e),
	function() {
		this.enter = function(a, b) {
			...
			var c = b.translatePosition && a.getCursorPosition(),
				d = c && this.occurToOriginalPosition(a.session, c);
			...
			var c = this.matchingLines(a.session, b),
				d = c.map(function(a) {
					return a.content
				}),
				e = new f(d.join("\n"));
...
}), ace.define("ace/keyboard/vim", ["require", "exports", "module", "ace/keyboard/vim/commands", "ace/keyboard/vim/maps/util", "ace/lib/useragent"], function(a, b) {
	var c = a("./vim/commands"),
		d = c.coreCommands,
		e = a("./vim/maps/util"),
		f = a("../lib/useragent"),
		g = {
	...
	b.handler = {
		...
		handleMacRepeat: function(a, b, c) {
			if (-1 == b) a.inputChar = c, a.lastEvent = "input";
			else if (a.inputChar && a.$lastHash == b && a.$lastKey == c) {
				if ("input" == a.lastEvent) a.lastEvent = "input1";
				else if ("input1" == a.lastEvent) return !0
			} else a.$lastHash = b, a.$lastKey = c, a.lastEvent = "keypress"
		},
		updateMacCompositionHandlers: function(a, b) {
			var c = function(a) {
					if ("insert" !== e.currentMode) {
						var b = this.textInput.getElement();
						b.blur(), b.focus(), b.value = a
					} else this.onCompositionUpdateOrig(a)
				},
				d = function(a) {
					"insert" === e.currentMode && this.onCompositionStartOrig(a)
				};
		...
		handleKeyboard: function(a, b, e, h) {
			if (0 !== b && (!e || -1 == h)) return null;
			var i = a.editor;
			if (1 == b && (e = "ctrl-" + e), "ctrl-c" == e) return !f.isMac && i.getCopyText() ? (i.once("copy", function() {
				"start" == a.state ? d.stop.exec(i) : i.selection.clearSelection()
			}), {
				command: "null",
				passEvent: !0
			}) : {
			...
			if ("esc" == e && 0 === b || "ctrl-[" == e) return {
				command: d.stop
			};
			if ("start" == a.state) {
				if (f.isMac && this.handleMacRepeat(a, b, e) && (b = -1, e = a.inputChar), -1 == b || 1 == b || 0 === b && e.length > 1) {
					if (c.inputBuffer.idle && g[e]) return g[e];
					var j = c.inputBuffer.push(i, e);
					return {
						command: "null",
						passEvent: !j
					}
				}
				if (1 == e.length && (0 === b || 4 == b)) return {
					command: "null",
					passEvent: !0
				};
				if ("esc" == e && 0 === b) return {
			...
			} else if ("ctrl-w" == e) return {
				command: "removewordleft"
			}
		},
		attach: function(a) {
			a.on("click", b.onCursorMove), "insert" !== e.currentMode && c.coreCommands.stop.exec(a), a.$vimModeHandler = this, this.updateMacCompositionHandlers(a, !0)
		},
		detach: function(a) {
			a.removeListener("click", b.onCursorMove), e.noMode(a), e.currentMode = "normal", this.updateMacCompositionHandlers(a, !1)
		},
		actions: c.actions,
		getStatusText: function() {
			return "insert" == e.currentMode ? "INSERT" : e.onVisualMode ? (e.onVisualLineMode ? "VISUAL LINE " : "VISUAL ") + c.inputBuffer.status : c.inputBuffer.status
		}
	}, b.onCursorMove = function(a) {
		c.onCursorMove(a.editor, a), b.onCursorMove.scheduled = !1
	}
}), ace.define("ace/keyboard/vim/commands", ["require", "exports", "module", "ace/lib/lang", "ace/keyboard/vim/maps/util", "ace/keyboard/vim/maps/motions", "ace/keyboard/vim/maps/operators", "ace/keyboard/vim/maps/aliases", "ace/keyboard/vim/registers"], function(a, b) {
	"never use strict";
	...
	var d = a("../../lib/lang"),
		...
		n = 8,
		o = function(a, b, c) {
			for (; 0 < b--;) a.apply(this, c)
		},
		p = function(a) {
			var b = a.renderer,
				c = b.$cursorLayer.getPixelPosition(),
				d = c.top,
				e = n * b.layerConfig.lineHeight;
			2 * e > b.$size.scrollerHeight && (e = b.$size.scrollerHeight / 2), b.scrollTop > d - e && b.session.setScrollTop(d - e), b.scrollTop + b.$size.scrollerHeight < d + e + b.lineHeight && b.session.setScrollTop(d + e + b.lineHeight - b.$size.scrollerHeight)
		},
		q = b.actions = {
			z: {
				param: !0,
				fn: function(a, b, c, d) {
					switch (d) {
						case "z":
							a.renderer.alignCursor(null, .5);
							break;
						case "t":
							a.renderer.alignCursor(null, 0);
							break;
						case "b":
							a.renderer.alignCursor(null, 1);
							break;
						case "c":
							a.session.onFoldWidgetClick(b.start.row, {
								domEvent: {
									target: {}
								}
							});
			...
			R: {
				fn: function(a) {
					e.insertMode(a), a.setOverwrite(!0)
				}
			},
			"~": {
				fn: function(a, b, c) {
					o(function() {
						...
						b.isEmpty() && b.end.column++;
						var c = a.session.getTextRange(b),
							d = c.toUpperCase();
						d == c ? a.navigateRight() : a.session.replace(b, d)
					}, c || 1)
				}
			},
			"*": {
				fn: function(a) {
					a.selection.selectWord(), a.findNext(), p(a);
					var b = a.selection.getRange();
					a.selection.setSelectionRange(b, !0)
				}
			},
			"#": {
				fn: function(a) {
					a.selection.selectWord(), a.findPrevious(), p(a);
					var b = a.selection.getRange();
					a.selection.setSelectionRange(b, !0)
				}
			},
			m: {
				...
				fn: function(a, b, c, d) {
					var e = a.session,
						f = e.vimMarkers || (e.vimMarkers = {}),
						g = a.getCursorPosition();
					f[d] || (f[d] = a.session.doc.createAnchor(g)), f[d].setPosition(g.row, g.column, !0)
				}
			},
			n: {
				fn: function(a) {
					var b = a.getLastSearchOptions();
					b.backwards = !1, a.selection.moveCursorRight(), a.selection.clearSelection(), a.findNext(b), p(a);
					var c = a.selection.getRange();
					c.end.row = c.start.row, c.end.column = c.start.column, a.selection.setSelectionRange(c, !0)
				}
			},
			N: {
				fn: function(a) {
					var b = a.getLastSearchOptions();
					b.backwards = !0, a.findPrevious(b), p(a);
					var c = a.selection.getRange();
					c.end.row = c.start.row, c.end.column = c.start.column, a.selection.setSelectionRange(c, !0)
				}
			},
			v: {
				fn: function(a) {
					a.selection.selectRight(), e.visualMode(a, !1)
				},
				acceptsMotion: !0
			},
			V: {
				fn: function(a) {
					var b = a.getCursorPosition().row;
					a.selection.moveTo(b, 0), a.selection.selectLineEnd(), a.selection.visualLineStart = b, e.visualMode(a, !0)
				},
				acceptsMotion: !0
			},
			Y: {
			...
			p: {
				fn: function(a, b, c) {
					var e = i._default;
					if (a.setOverwrite(!1), e.isLine) {
						var f = a.getCursorPosition();
						f.column = a.session.getLine(f.row).length;
						var g = d.stringRepeat("\n" + e.text, c || 1);
						a.session.insert(f, g), a.moveCursorTo(f.row + 1, 0)
					} else a.navigateRight(), a.insert(d.stringRepeat(e.text, c || 1)), a.navigateLeft();
					a.setOverwrite(!0), a.selection.clearSelection()
				}
			},
			P: {
				fn: function(a, b, c) {
					...
					if (a.setOverwrite(!1), e.isLine) {
						var f = a.getCursorPosition();
						f.column = 0;
						var g = d.stringRepeat(e.text + "\n", c || 1);
						a.session.insert(f, g), a.moveCursorToPosition(f)
					} else a.insert(d.stringRepeat(e.text, c || 1));
					a.setOverwrite(!0), a.selection.clearSelection()
				}
			},
			J: {
				fn: function(a, b, c) {
					...
					c = c || b.end.row - b.start.row;
					var f = Math.min(e.row + (c || 1), d.getLength() - 1);
					b.start.column = d.getLine(e.row).length, b.end.column = d.getLine(f).length, b.end.row = f;
					for (var g = "", h = e.row; f > h; h++) {
						var i = d.getLine(h + 1);
						g += " " + /^\s*(.*)$/.exec(i)[1] || ""
					}
					d.replace(b, g), a.moveCursorTo(e.row, e.column)
				}
			},
			u: {
				fn: function(a, b, c) {
					c = parseInt(c || 1, 10);
					for (var d = 0; c > d; d++) a.undo();
			...
			":": {
				fn: function(a, b, c) {
					var d = ":";
					c > 1 && (d = ".,.+" + c + d), a.showCommandLine && a.showCommandLine(d)
				}
			},
			"/": {
				fn: function(a) {
					a.showCommandLine && a.showCommandLine("/")
				}
			},
			"?": {
				fn: function(a) {
			...
			".": {
				fn: function(a) {
					e.onInsertReplaySequence = r.lastInsertCommands;
					var b = r.previous;
					b && r.exec(a, b.action, b.param)
				}
			},
			"ctrl-x": {
				fn: function(a, b, c) {
		...
		r = b.inputBuffer = {
			...
			push: function(a, b) {
				var c = this.status,
					d = !0;
				this.idle = !1;
				var e = this.waitingForParam;
				if (/^numpad\d+$/i.test(b) && (b = b.substr(6)), e) this.exec(a, e, b);
				else if (("0" !== b || this.currentCount.length) && /^\d+$/.test(b) && this.isAccepting(j)) this.currentCount += b, this.currentCmd = j, this.accepting = [j, k, l, m];
				else if (!this.operator && this.isAccepting(k) && g[b]) this.operator = {
					ch: b,
					count: this.getCount()
				}, this.currentCmd = k, this.accepting = [j, l, m], this.exec(a, {
					operator: this.operator
				});
				else if (f[b] && this.isAccepting(l)) {
					this.currentCmd = l;
					var i = {
						operator: this.operator,
						motion: {
					...
					f[b].param ? this.waitForParam(i) : this.exec(a, i)
				} else if (h[b] && this.isAccepting(l)) h[b].operator.count = this.getCount(), this.exec(a, h[b]);
				else if (q[b] && this.isAccepting(m)) {
					var n = {
						action: {
							fn: q[b].fn,
							count: this.getCount()
					...
					q[b].param ? this.waitForParam(n) : this.exec(a, n), q[b].acceptsMotion && (this.idle = !1)
				} else this.operator ? (this.operator.count = this.getCount(), this.exec(a, {
					operator: this.operator
				}, b)) : (d = 1 == b.length, this.reset());
				return this.waitingForParam || this.motion || this.operator ? this.status += b : this.currentCount ? this.status = this.currentCount : this.status && (this.status = ""), this.status != c && a._emit("changeStatus"), d
			},
			waitForParam: function(a) {
				this.waitingForParam = a
			},
			getCount: function() {
				var a = this.currentCount;
				return this.currentCount = "", a && parseInt(a, 10)
			},
			exec: function(a, b, c) {
				var d = b.motion,
					h = b.operator,
					i = b.action;
				if (c || (c = b.param), h && (this.previous = {
					action: b,
					param: c
				}), h && !a.selection.isEmpty()) return void(g[h.ch].selFn && (g[h.ch].selFn(a, a.getSelectionRange(), h.count, c), this.reset()));
				if (!d && !i && h && c) g[h.ch].fn(a, null, h.count, c), this.reset();
				else if (d) {
					var j = function(b) {
							b && "function" == typeof b && (d.count && !k.handlesCount ? o(b, d.count, [a, null, d.count, c]) : b(a, null, d.count, c))
						},
						k = f[d.ch],
						l = k.sel;
					h ? l && o(function() {
						j(k.sel), g[h.ch].fn(a, a.getSelectionRange(), h.count, c)
					}, h.count || 1) : j((e.onVisualMode || e.onVisualLineMode) && l ? k.sel : k.nav), this.reset()
				} else i && (i.fn(a, a.getSelectionRange(), i.count, c), this.reset());
				s(a)
			},
			isAccepting: function(a) {
				return -1 !== this.accepting.indexOf(a)
			},
			reset: function() {
				this.operator = null, this.motion = null, this.currentCount = "", this.status = "", this.accepting = [j, k, l, m], this.idle = !0, this.waitingForParam = null
			}
		};
	b.coreCommands = {
		start: {
		...
		startBeginning: {
			exec: function u(a) {
				a.navigateLineStart(), e.insertMode(a), c(u)
			}
		},
		stop: {
			exec: function(a) {
				r.reset(), e.onVisualMode = !1, e.onVisualLineMode = !1, r.lastInsertCommands = e.normalMode(a)
			}
		},
		append: {
			exec: function v(a) {
				var b = a.getCursorPosition(),
					d = a.session.getLine(b.row).length;
				d && a.navigateRight(), e.insertMode(a), c(v)
			}
		},
		appendEnd: {
			exec: function w(a) {
				a.navigateLineEnd(), e.insertMode(a), c(w)
			}
		}
	};
	var s = b.onCursorMove = function(a, b) {
		if ("insert" !== e.currentMode && !s.running) {
			if (!a.selection.isEmpty()) {
				if (s.running = !0, e.onVisualLineMode) {
					var c = a.selection.visualLineStart,
						d = a.getCursorPosition().row;
					if (d >= c) {
						var f = a.session.getLine(d);
						a.selection.moveTo(c, 0), a.selection.selectTo(d, f.length)
					} else {
						var f = a.session.getLine(c);
						a.selection.moveTo(c, f.length), a.selection.selectTo(d, 0)
					}
				}
				return void(s.running = !1)
			}
			b && (e.onVisualLineMode || e.onVisualMode) && (a.selection.clearSelection(), e.normalMode(a)), s.running = !0;
			var g = a.getCursorPosition(),
				h = a.session.getLine(g.row).length;
			h && g.column === h && a.navigateLeft(), s.running = !1
		}
	}
}), ace.define("ace/keyboard/vim/maps/util", ["require", "exports", "module", "ace/keyboard/vim/registers", "ace/lib/dom"], function(a, b, c) {
	var d = a("../registers"),
		e = a("../../../lib/dom");
	e.importCssString(".insert-mode .ace_cursor{    border-left: 2px solid #333333;}.ace_dark.insert-mode .ace_cursor{    border-left: 2px solid #eeeeee;}.normal-mode .ace_cursor{    border: 0!important;    background-color: red;    opacity: 0.5;}", "vimMode"), c.exports = {
		onVisualMode: !1,
		onVisualLineMode: !1,
		currentMode: "normal",
		noMode: function(a) {
			a.unsetStyle("insert-mode"), a.unsetStyle("normal-mode"), a.commands.recording && a.commands.toggleRecording(a), a.setOverwrite(!1)
		},
		insertMode: function(a) {
			this.currentMode = "insert", a.setStyle("insert-mode"), a.unsetStyle("normal-mode"), a.setOverwrite(!1), a.keyBinding.$data.buffer = "", a.keyBinding.$data.state = "insertMode", this.onVisualMode = !1, this.onVisualLineMode = !1, this.onInsertReplaySequence ? (a.commands.macro = this.onInsertReplaySequence, a.commands.replay(a), this.onInsertReplaySequence = null, this.normalMode(a)) : (a._emit("changeStatus"), a.commands.recording || a.commands.toggleRecording(a))
		},
		normalMode: function(a) {
			this.currentMode = "normal", a.unsetStyle("insert-mode"), a.setStyle("normal-mode"), a.clearSelection();
			var b;
			return a.getOverwrite() || (b = a.getCursorPosition(), b.column > 0 && a.navigateLeft()), a.setOverwrite(!0), a.keyBinding.$data.buffer = "", a.keyBinding.$data.state = "start", this.onVisualMode = !1, this.onVisualLineMode = !1, a._emit("changeStatus"), a.commands.recording ? (a.commands.toggleRecording(a), a.commands.macro) : []
		},
		visualMode: function(a, b) {
			return this.onVisualLineMode && b || this.onVisualMode && !b ? void this.normalMode(a) : (a.setStyle("insert-mode"), a.unsetStyle("normal-mode"), a._emit("changeStatus"), void(b ? this.onVisualLineMode = !0 : (this.onVisualMode = !0, this.onVisualLineMode = !1)))
		},
		getRightNthChar: function(a, b, c, d) {
			var e = a.getSession().getLine(b.row),
				f = e.substr(b.column + 1).split(c);
			return d < f.length ? f.slice(0, d).join(c).length : null
		},
		getLeftNthChar: function(a, b, c, d) {
			var e = a.getSession().getLine(b.row),
				f = e.substr(0, b.column).split(c);
			return d < f.length ? f.slice(-1 * d).join(c).length : null
		},
		toRealChar: function(a) {
			return 1 === a.length ? a : /^shift-./.test(a) ? a[a.length - 1].toUpperCase() : ""
		},
		copyLine: function(a) {
			var b = a.getCursorPosition();
			a.selection.moveTo(b.row, b.column), a.selection.selectLine(), d._default.isLine = !0, d._default.text = a.getCopyText().replace(/\n$/, ""), a.selection.moveTo(b.row, b.column)
		}
	}
}), ace.define("ace/keyboard/vim/registers", ["require", "exports", "module"], function(a, b, c) {
	"never use strict";
	c.exports = {
		_default: {
			text: "",
...
}), ace.define("ace/keyboard/vim/maps/motions", ["require", "exports", "module", "ace/keyboard/vim/maps/util", "ace/search", "ace/range"], function(a, b, c) {
	function d(a) {
		if ("function" == typeof a) {
			var b = a;
			a = this
		} else var b = a.getPos;
		return a.nav = function(a, c, d, e) {
			var f = b(a, c, d, e, !1);
			f && a.selection.moveTo(f.row, f.column)
		}, a.sel = function(a, c, d, e) {
			var f = b(a, c, d, e, !0);
			f && a.selection.selectTo(f.row, f.column)
		}, a
	}

	function e(a, b, c) {
		return m.$options.needle = b, m.$options.backwards = -1 == c, m.find(a.session)
	}
	var f = a("./util"),
		g = function(a, b) {
			var c = a.renderer.getScrollTopRow(),
				d = a.getCursorPosition().row,
				e = d - c;
			b && b.call(a), a.renderer.scrollToRow(a.getCursorPosition().row - e)
		},
		h = /[\s.\/\\()\"'-:,.;<>~!@#$%^&*|+=\[\]{}`~?]/,
		i = /[.\/\\()\"'-:,.;<>~!@#$%^&*|+=\[\]{}`~?]/,
		j = /\s/,
		k = function(a, b) {
			var c = a.selection;
			this.range = c.getRange(), b = b || c.selectionLead, this.row = b.row, this.col = b.column;
			var d = a.session.getLine(this.row),
				e = a.session.getLength();
			this.ch = d[this.col] || "\n", this.skippedLines = 0, this.next = function() {
				return this.ch = d[++this.col] || this.handleNewLine(1), this.ch
			}, this.prev = function() {
				return this.ch = d[--this.col] || this.handleNewLine(-1), this.ch
			}, this.peek = function(b) {
				var c = d[this.col + b];
				return c ? c : -1 == b ? "\n" : this.col == d.length - 1 ? "\n" : a.session.getLine(this.row + 1)[0] || "\n"
			}, this.handleNewLine = function(b) {
				return 1 == b ? this.col == d.length ? "\n" : this.row == e - 1 ? "" : (this.col = 0, this.row++, d = a.session.getLine(this.row), this.skippedLines++, d[0] || "\n") : -1 == b ? 0 === this.row ? "" : (this.row--, d = a.session.getLine(this.row), this.col = d.length, this.skippedLines--, "\n") : void 0
			}, this.debug = function() {
				console.log(d.substring(0, this.col) + "|" + this.ch + "'" + this.col + "'" + d.substr(this.col + 1))
			}
		},
		l = a("../../../search").Search,
		m = new l,
	...
	c.exports = {
		w: new d(function(a) {
			var b = new k(a);
			if (b.ch && i.test(b.ch))
				for (; b.ch && i.test(b.ch);) b.next();
			else
				for (; b.ch && !h.test(b.ch);) b.next();
			for (; b.ch && j.test(b.ch) && b.skippedLines < 2;) b.next();
			return 2 == b.skippedLines && b.prev(), {
				column: b.col,
				row: b.row
			}
		}),
		W: new d(function(a) {
			for (var b = new k(a); b.ch && (!j.test(b.ch) || j.test(b.peek(1))) && b.skippedLines < 2;) b.next();
			return 2 == b.skippedLines ? b.prev() : b.next(), {
				column: b.col,
				row: b.row
			}
		}),
		b: new d(function(a) {
			var b = new k(a);
			for (b.prev(); b.ch && j.test(b.ch) && b.skippedLines > -2;) b.prev();
			if (b.ch && i.test(b.ch))
				for (; b.ch && i.test(b.ch);) b.prev();
			else
				for (; b.ch && !h.test(b.ch);) b.prev();
			return b.ch && b.next(), {
				column: b.col,
				row: b.row
			}
		...
		B: new d(function(a) {
			var b = new k(a);
			for (b.prev(); b.ch && (j.test(b.ch) || !j.test(b.peek(-1))) && b.skippedLines > -2;) b.prev();
			return -2 == b.skippedLines && b.next(), {
				column: b.col,
				row: b.row
			}
		}),
		e: new d(function(a) {
			var b = new k(a);
			for (b.next(); b.ch && j.test(b.ch);) b.next();
			if (b.ch && i.test(b.ch))
				for (; b.ch && i.test(b.ch);) b.next();
			else
				for (; b.ch && !h.test(b.ch);) b.next();
		...
		E: new d(function(a) {
			var b = new k(a);
			for (b.next(); b.ch && (j.test(b.ch) || !j.test(b.peek(1)));) b.next();
			return {
				column: b.col,
				row: b.row
			}
		...
		l: {
			nav: function(a) {
				var b = a.getCursorPosition(),
					c = b.column,
					d = a.session.getLine(b.row).length;
				d && c !== d && a.navigateRight()
			},
			sel: function(a) {
				var b = a.getCursorPosition(),
					c = b.column,
					d = a.session.getLine(b.row).length;
				d && c !== d && a.selection.selectRight()
			}
		},
		h: {
			nav: function(a) {
				var b = a.getCursorPosition();
				b.column > 0 && a.navigateLeft()
			},
			sel: function(a) {
				var b = a.getCursorPosition();
				b.column > 0 && a.selection.selectLeft()
			}
		},
		H: {
			nav: function(a) {
				var b = a.renderer.getScrollTopRow();
				a.moveCursorTo(b)
			},
			sel: function(a) {
				var b = a.renderer.getScrollTopRow();
				a.selection.selectTo(b)
			}
		},
		M: {
			nav: function(a) {
				var b = a.renderer.getScrollTopRow(),
					c = a.renderer.getScrollBottomRow(),
					d = b + (c - b) / 2;
				a.moveCursorTo(d)
			},
			sel: function(a) {
				var b = a.renderer.getScrollTopRow(),
					c = a.renderer.getScrollBottomRow(),
					d = b + (c - b) / 2;
				a.selection.selectTo(d)
			}
		},
		L: {
			nav: function(a) {
				var b = a.renderer.getScrollBottomRow();
				a.moveCursorTo(b)
			},
			sel: function(a) {
				var b = a.renderer.getScrollBottomRow();
			...
			sel: function(a, b, c, d) {
				switch (d) {
					case "w":
						...
						break;
					case "W":
						a.selection.selectAWord();
						break;
					case "(":
					case "{":
					case "[":
						var f = a.getCursorPosition(),
							g = a.session.$findClosingBracket(d, f, /paren/);
						if (!g) return;
						var h = a.session.$findOpeningBracket(a.session.$brackets[d], f, /paren/);
						if (!h) return;
						h.column++, a.selection.setSelectionRange(n.fromPoints(h, g));
						break;
					case "'":
					case '"':
					case "/":
						...
						var h = e(a, d, -1);
						if (!h) return;
						a.selection.setSelectionRange(n.fromPoints(h.end, g.start))
				}
			}
		},
		a: {
			...
			sel: function(a, b, c, d) {
				switch (d) {
					...
					case "[":
						...
						var h = a.session.$findOpeningBracket(a.session.$brackets[d], f, /paren/);
						if (!h) return;
						g.column++, a.selection.setSelectionRange(n.fromPoints(h, g));
						break;
					case "'":
					case '"':
					case "/":
						...
						var h = e(a, d, -1);
						if (!h) return;
						g.column++, a.selection.setSelectionRange(n.fromPoints(h.start, g.end))
				}
			}
		},
		f: new d({
			param: !0,
			handlesCount: !0,
			getPos: function(a, b, c, d, e, g) {
				g || (o = {
					ch: "f",
					param: d
				});
				var h = a.getCursorPosition(),
					i = f.getRightNthChar(a, h, d, c || 1);
				return "number" == typeof i ? (h.column += i + (e ? 2 : 1), h) : void 0
			}
		}),
		F: new d({
			param: !0,
			...
			getPos: function(a, b, c, d, e, g) {
				...
				var h = a.getCursorPosition(),
					i = f.getLeftNthChar(a, h, d, c || 1);
				return "number" == typeof i ? (h.column -= i + 1, h) : void 0
			}
		}),
		t: new d({
			param: !0,
			...
			getPos: function(a, b, c, d, e, g) {
				...
				var h = a.getCursorPosition(),
					i = f.getRightNthChar(a, h, d, c || 1);
				if (g && 0 == i && !(c > 1)) var i = f.getRightNthChar(a, h, d, 2);
				return "number" == typeof i ? (h.column += i + (e ? 1 : 0), h) : void 0
			}
		}),
		T: new d({
			param: !0,
			...
			getPos: function(a, b, c, d, e, g) {
				...
				var h = a.getCursorPosition(),
					i = f.getLeftNthChar(a, h, d, c || 1);
				if (g && 0 == i && !(c > 1)) var i = f.getLeftNthChar(a, h, d, 2);
				return "number" == typeof i ? (h.column -= i, h) : void 0
			}
		}),
		";": new d({
			handlesCount: !0,
			getPos: function(a, b, d, e, f) {
				var g = o.ch;
				if (g) return c.exports[g].getPos(a, b, d, o.param, f, !0)
			}
		}),
		",": new d({
			handlesCount: !0,
			getPos: function(a, b, d, e, f) {
				...
				if (g) {
					var h = g.toUpperCase();
					return g = g === h ? g.toLowerCase() : h, c.exports[g].getPos(a, b, d, o.param, f, !0)
				}
			}
		}),
		"^": {
		...
		$: {
			...
			sel: function(a, b, c) {
				c > 1 && a.selection.moveCursorBy(c - 1, 0), a.selection.selectLineEnd()
			}
		},
		0: new d(function(a) {
			return {
				row: a.selection.lead.row,
				column: 0
			}
		}),
		G: {
			nav: function(a, b, c) {
				c || 0 === c || (c = a.session.getLength()), a.gotoLine(c)
			},
			sel: function(a, b, c) {
				c || 0 === c || (c = a.session.getLength()), a.selection.selectTo(c, 0)
			}
		},
		g: {
			param: !0,
		...
		o: {
			nav: function(a, b, c) {
				c = c || 1;
				for (var d = ""; 0 < c--;) d += "\n";
				d.length && (a.navigateLineEnd(), a.insert(d), f.insertMode(a))
			}
		},
		O: {
			nav: function(a, b, c) {
				var d = a.getCursorPosition().row;
				c = c || 1;
				for (var e = ""; 0 < c--;) e += "\n";
				e.length && (d > 0 ? (a.navigateUp(), a.navigateLineEnd(), a.insert(e)) : (a.session.insert({
					row: 0,
					column: 0
				}, e), a.navigateUp()), f.insertMode(a))
			}
		},
		"%": new d(function(a) {
			var b = /[\[\]{}()]/g,
				c = a.getCursorPosition(),
				d = a.session.getLine(c.row)[c.column];
			if (!b.test(d)) {
				var f = e(a, b);
				if (!f) return;
				c = f.start
			}
			var g = a.session.findMatchingBracket({
				row: c.row,
				column: c.column + 1
			});
			return g
		...
		"{": new d(function(a) {
			for (var b = a.session, c = b.selection.lead.row; c > 0 && !/\S/.test(b.getLine(c));) c--;
			for (;
				/\S/.test(b.getLine(c));) c--;
			return {
				column: 0,
				row: c
			}
		...
		"}": new d(function(a) {
			for (var b = a.session, c = b.getLength(), d = b.selection.lead.row; c > d && !/\S/.test(b.getLine(d));) d++;
			for (;
				/\S/.test(b.getLine(d));) d++;
			return {
				column: 0,
				row: d
			}
		...
		"ctrl-d": {
			...
			sel: function(a) {
				g(a, a.selectPageDown)
			}
		},
		"ctrl-u": {
			nav: function(a) {
				a.selection.clearSelection(), g(a, a.gotoPageUp)
			},
			sel: function(a) {
				g(a, a.selectPageUp)
			}
		...
		"`": new d({
			param: !0,
			handlesCount: !0,
			getPos: function(a, b, c, d) {
				var e = a.session,
					f = e.vimMarkers && e.vimMarkers[d];
				return f ? f.getPosition() : void 0
			}
		}),
		"'": new d({
			param: !0,
			...
			getPos: function(a, b, c, d) {
				...
				if (f) {
					var g = f.getPosition(),
						h = a.session.getLine(g.row);
					return g.column = h.search(/\S/), -1 == g.column && (g.column = h.length), g
				}
			}
		})
	}, c.exports.backspace = c.exports.left = c.exports.h, c.exports.space = c.exports["return"] = c.exports.right = c.exports.l, c.exports.up = c.exports.k, c.exports.down = c.exports.j, c.exports.pagedown = c.exports["ctrl-d"], c.exports.pageup = c.exports["ctrl-u"], c.exports.home = c.exports[0], c.exports.end = c.exports.$
}), ace.define("ace/keyboard/vim/maps/operators", ["require", "exports", "module", "ace/keyboard/vim/maps/util", "ace/keyboard/vim/registers"], function(a, b, c) {
	var d = a("./util"),
		e = a("../registers");
	c.exports = {
		d: {
			selFn: function(a, b) {
				e._default.text = a.getCopyText(), e._default.isLine = d.onVisualLineMode, d.onVisualLineMode ? a.removeLines() : a.session.remove(b), d.normalMode(a)
			},
			fn: function(a, b, c, d) {
				switch (c = c || 1, d) {
					case "d":
						e._default.text = "", e._default.isLine = !0;
						for (var f = 0; c > f; f++) {
							a.selection.selectLine(), e._default.text += a.getCopyText();
							var g = a.getSelectionRange();
							if (!g.isMultiLine()) {
								var h = g.start.row - 1,
									i = a.session.getLine(h).length;
								g.setStart(h, i), a.session.remove(g), a.selection.clearSelection();
								break
							}
							a.session.remove(g), a.selection.clearSelection()
						}
						e._default.text = e._default.text.replace(/\n$/, "");
						break;
					default:
						b && (a.selection.setSelectionRange(b), e._default.text = a.getCopyText(), e._default.isLine = !1, a.session.remove(b), a.selection.clearSelection())
				}
			}
		},
		c: {
			selFn: function(a, b) {
				a.session.remove(b), d.insertMode(a)
			},
			fn: function(a, b, c, e) {
				switch (c = c || 1, e) {
					case "c":
						for (var f = 0; c > f; f++) a.removeLines(), d.insertMode(a);
						break;
					default:
						b && (a.session.remove(b), d.insertMode(a))
				}
			}
		},
		y: {
			selFn: function(a) {
				e._default.text = a.getCopyText(), e._default.isLine = d.onVisualLineMode, a.selection.clearSelection(), d.normalMode(a)
			},
			fn: function(a, b, c, d) {
				switch (c = c || 1, d) {
					case "y":
						...
						for (var g = 0; c - 1 > g; g++) a.selection.moveCursorDown();
						e._default.text = a.getCopyText().replace(/\n$/, ""), a.selection.clearSelection(), e._default.isLine = !0, a.moveCursorToPosition(f);
						break;
					default:
						if (b) {
							var f = a.getCursorPosition();
							a.selection.setSelectionRange(b), e._default.text = a.getCopyText(), e._default.isLine = !1, a.selection.clearSelection(), a.moveCursorTo(f.row, f.column)
						}
				}
			}
		},
		">": {
			selFn: function(a, b, c) {
				c = c || 1;
				for (var e = 0; c > e; e++) a.indent();
				d.normalMode(a)
			},
			fn: function(a, b, c, d) {
				switch (c = parseInt(c || 1, 10), d) {
					case ">":
						var e = a.getCursorPosition();
						a.selection.selectLine();
						for (var f = 0; c - 1 > f; f++) a.selection.moveCursorDown();
						a.indent(), a.selection.clearSelection(), a.moveCursorToPosition(e), a.navigateLineEnd(), a.navigateLineStart()
				}
			}
		},
		"<": {
			selFn: function(a, b, c) {
				c = c || 1;
				for (var e = 0; c > e; e++) a.blockOutdent();
				d.normalMode(a)
			},
			fn: function(a, b, c, d) {
				switch (c = c || 1, d) {
					case "<":
						...
						a.selection.selectLine();
						for (var f = 0; c - 1 > f; f++) a.selection.moveCursorDown();
						a.blockOutdent(), a.selection.clearSelection(), a.moveCursorToPosition(e), a.navigateLineEnd(), a.navigateLineStart()
				}
			}
		}
	}
}), ace.define("ace/keyboard/vim/maps/aliases", ["require", "exports", "module"], function(a, b, c) {
	c.exports = {
		x: {
			operator: {
				ch: "d",
...
}), ace.define("ace/mode/markdown", ["require", "exports", "module", "ace/lib/oop", "ace/mode/text", "ace/mode/javascript", "ace/mode/xml", "ace/mode/html", "ace/tokenizer", "ace/mode/markdown_highlight_rules", "ace/mode/folding/markdown", "ace/mode/behaviour/cstyle"], function(a, b) {
	var c = a("../lib/oop"),
		d = a("./text").Mode,
		e = a("./javascript").Mode,
		f = a("./xml").Mode,
		g = a("./html").Mode,
		h = (a("../tokenizer").Tokenizer, a("./markdown_highlight_rules").MarkdownHighlightRules),
		i = a("./folding/markdown").FoldMode,
		j = a("./behaviour/cstyle").CstyleBehaviour,
		k = function() {
			this.HighlightRules = h, this.createModeDelegates({
				"js-": e,
				"xml-": f,
				"html-": g
			}), this.$behaviour = new j, this.foldingRules = new i
		};
	c.inherits(k, d),
	function() {
		this.type = "text", this.lineCommentStart = ">";
		var a = this.transformAction;
		this.transformAction = function(b, c, d, e, f) {
			if ("listblock-start" == b && "insertion" === c && "\n" === f) {
				var g = d.getSelectionRange().start,
					h = /^[\t\s]*(([-+*])|(\d+)\.)\s+$/.exec(e.doc.getLine(g.row));
				if (h) return e.doc.removeLines(g.row, g.row), e.doc.insertLines(g.row, [g.row >= e.doc.$lines.length - 1 ? "\n" : ""]), {
					text: ""
				}
			}
			...
			if ("listblock" == a) {
				...
				if (!c) return "";
				var d = c[2];
				return d || (d = parseInt(c[3], 10) + 1 + "."), c[1] + d + c[4]
			}
			return this.$getIndent(b)
		}, this.$id = "ace/mode/markdown"
	}.call(k.prototype), b.Mode = k
}), ace.define("ace/mode/javascript", ["require", "exports", "module", "ace/lib/oop", "ace/mode/text", "ace/tokenizer", "ace/mode/javascript_highlight_rules", "ace/mode/matching_brace_outdent", "ace/range", "ace/worker/worker_client", "ace/mode/behaviour/cstyle", "ace/mode/folding/cstyle"], function(a, b) {
	var c = a("../lib/oop"),
		d = a("./text").Mode,
		e = (a("../tokenizer").Tokenizer, a("./javascript_highlight_rules").JavaScriptHighlightRules),
		f = a("./matching_brace_outdent").MatchingBraceOutdent,
		g = (a("../range").Range, a("../worker/worker_client").WorkerClient),
		h = a("./behaviour/cstyle").CstyleBehaviour,
		i = a("./folding/cstyle").FoldMode,
		j = function() {
...
}), ace.define("ace/mode/javascript_highlight_rules", ["require", "exports", "module", "ace/lib/oop", "ace/mode/doc_comment_highlight_rules", "ace/mode/text_highlight_rules"], function(a, b) {
	var c = a("../lib/oop"),
		d = a("./doc_comment_highlight_rules").DocCommentHighlightRules,
		...
		f = function() {
			var a = this.createKeywordMapper({
				...
				b = "case|do|else|finally|in|instanceof|return|throw|try|typeof|yield|void",
				c = "[a-zA-Z\\$_¡-￿][a-zA-Z\\d\\$_¡-￿]*\\b",
				e = "\\\\(?:x[0-9a-fA-F]{2}|u[0-9a-fA-F]{4}|[0-2][0-7]{0,2}|3[0-6][0-7]?|37[0-7]?|[4-7][0-7]?|.)";
			this.$rules = {
				no_regex: [{
						token: "comment",
						regex: "\\/\\/",
			...
			}, this.embedRules(d, "doc-", [d.getEndRule("no_regex")])
		};
	c.inherits(f, e), b.JavaScriptHighlightRules = f
}), ace.define("ace/mode/doc_comment_highlight_rules", ["require", "exports", "module", "ace/lib/oop", "ace/mode/text_highlight_rules"], function(a, b) {
	var c = a("../lib/oop"),
		d = a("./text_highlight_rules").TextHighlightRules,
		e = function() {
			this.$rules = {
	...
	c.inherits(e, d), e.getStartRule = function(a) {
		return {
			token: "comment.doc",
			regex: "\\/\\*(?=\\*)",
			next: a
...
}), ace.define("ace/mode/matching_brace_outdent", ["require", "exports", "module", "ace/range"], function(a, b) {
	var c = a("../range").Range,
		d = function() {};
	(function() {
...
}), ace.define("ace/mode/behaviour/cstyle", ["require", "exports", "module", "ace/lib/oop", "ace/mode/behaviour", "ace/token_iterator", "ace/lib/lang"], function(a, b) {
	var c = a("../../lib/oop"),
		d = a("../behaviour").Behaviour,
		e = a("../../token_iterator").TokenIterator,
		f = a("../../lib/lang"),
		g = ["text", "paren.rparen", "punctuation.operator", "notebook.string.comment"],
		h = ["text", "paren.rparen", "punctuation.operator", "comment", "notebook.string.comment"],
		i = 0,
		j = -1,
		k = "",
		l = 0,
		...
		o = "",
		p = function() {
			p.isSaneInsertion = function(a, b) {
				var c = a.getCursorPosition(),
					d = new e(b, c.row, c.column);
				if (!this.$matchTokenType(d.getCurrentToken() || "text", g)) {
					var f = new e(b, c.row, c.column + 1);
					if (!this.$matchTokenType(f.getCurrentToken() || "text", g)) return !1
				}
				return d.stepForward(), d.getCurrentTokenRow() !== c.row || this.$matchTokenType(d.getCurrentToken() || "text", h)
			}, p.$matchTokenType = function(a, b) {
				return b.indexOf(a.type || a) > -1
			}, p.recordAutoInsert = function(a, b, c) {
				var d = a.getCursorPosition(),
					e = b.doc.getLine(d.row);
				this.isAutoInsertedClosing(d, e, k[0]) || (i = 0), j = d.row, k = c + e.substr(d.column), i++
			}, p.recordMaybeInsert = function(a, b, c) {
				var d = a.getCursorPosition(),
					e = b.doc.getLine(d.row);
				this.isMaybeInsertedClosing(d, e) || (l = 0), m = d.row, n = e.substr(0, d.column) + c, o = e.substr(d.column), l++
			}, p.isAutoInsertedClosing = function(a, b, c) {
				return i > 0 && a.row === j && c === k[0] && b.substr(a.column) === k
			}, p.isMaybeInsertedClosing = function(a, b) {
				return l > 0 && a.row === m && b.substr(a.column) === o && b.substr(0, a.column) == n
				...
				var g = c.getCursorPosition(),
					h = d.doc.getLine(g.row);
				if ("{" == e) {
					var i = c.getSelectionRange(),
						j = d.doc.getTextRange(i);
					if ("" !== j && "{" !== j && c.getWrapBehavioursEnabled()) return {
						text: "{" + j + "}",
						selection: !1
					};
					if (p.isSaneInsertion(c, d)) return /[\]\}\)]/.test(h[g.column]) || c.inMultiSelectMode ? (p.recordAutoInsert(c, d, "}"), {
						text: "{}",
						selection: [1, 1]
					}) : (p.recordMaybeInsert(c, d, "{"), p.recordAutoInsert(c, d, "}"), {
						text: "{}",
						selection: [1, 1]
					})
				} else if ("}" == e) {
					var k = h.substring(g.column, g.column + 1);
					if ("}" == k) {
						var m = d.$findOpeningBracket("}", {
							column: g.column + 1,
							row: g.row
						});
						if (null !== m && p.isAutoInsertedClosing(g, h, e)) return p.popAutoInsertedClosing(), {
							text: "",
							selection: [1, 1]
						}
					}
					...
					if ("\n" == e || "\r\n" == e) {
						var n = "";
						p.isMaybeInsertedClosing(g, h) && (n = f.stringRepeat("}", l), p.clearMaybeInsertedClosing());
						var k = h.substring(g.column, g.column + 1);
						if ("}" === k) {
							var o = d.findMatchingBracket({
								row: g.row,
								column: g.column + 1
							}, "}");
							if (!o) return null;
							var q = this.$getIndent(d.getLine(o.row))
						} else {
							if (!n) return;
							var q = this.$getIndent(h)
						}
						var r = q + d.getTabString();
						return {
							text: "\n" + r + "\n" + q + n,
							selection: [1, r.length, 1, r.length]
						}
					}
				...
				if ("(" == e) {
					...
					if ("" !== g && c.getWrapBehavioursEnabled()) return {
						text: "(" + g + ")",
						selection: !1
					};
					if (p.isSaneInsertion(c, d)) return p.recordAutoInsert(c, d, ")"), {
						text: "()",
						selection: [1, 1]
					}
				} else if (")" == e) {
					...
					if (")" == j) {
						...
						if (null !== k && p.isAutoInsertedClosing(h, i, e)) return p.popAutoInsertedClosing(), {
							text: "",
							selection: [1, 1]
						}
					}
				...
				if ('"' == e || "'" == e) {
					...
					if ("" !== h && "'" !== h && '"' != h && c.getWrapBehavioursEnabled()) return {
						text: f + h + f,
						selection: !1
					};
					...
					if ("\\" == k) return null;
					if (/\b/.test(k)) return;
					for (var l, m = d.getTokens(g.start.row), n = 0, o = -1, q = 0; q < m.length && (l = m[q], "string" == l.type ? o = -1 : 0 > o && (o = l.value.indexOf(f)), !(l.value.length + n > g.start.column)); q++) n += m[q].value.length;
					if (!l || 0 > o && "comment" !== l.type && ("string" !== l.type || g.start.column !== l.value.length + n - 1 && l.value.lastIndexOf(f) === l.value.length - 1)) {
						if (!p.isSaneInsertion(c, d)) return;
						return {
							text: f + f,
							selection: [1, 1]
						}
					...
					if (l && "string" === l.type) {
						var r = j.substring(i.column, i.column + 1);
						if (r == f) return {
							text: "",
							selection: [1, 1]
						}
	...
	c.inherits(p, d), b.CstyleBehaviour = p
}), ace.define("ace/mode/folding/cstyle", ["require", "exports", "module", "ace/lib/oop", "ace/range", "ace/mode/folding/fold_mode"], function(a, b) {
	var c = a("../../lib/oop"),
		d = a("../../range").Range,
	...
	c.inherits(f, e),
	function() {
		this.foldingStartMarker = /(\{|\[)[^\}\]]*$|^\s*(\/\*)/, this.foldingStopMarker = /^[^\[\{]*(\}|\])|^[\s\*]*(\*\/)/, this.getFoldWidgetRange = function(a, b, c, d) {
			var e = a.getLine(c),
				f = e.match(this.foldingStartMarker);
			if (f) {
				var g = f.index;
...
}), ace.define("ace/mode/xml", ["require", "exports", "module", "ace/lib/oop", "ace/mode/text", "ace/tokenizer", "ace/mode/xml_highlight_rules", "ace/mode/behaviour/xml", "ace/mode/folding/xml"], function(a, b) {
	var c = a("../lib/oop"),
		d = a("./text").Mode,
		e = (a("../tokenizer").Tokenizer, a("./xml_highlight_rules").XmlHighlightRules),
		f = a("./behaviour/xml").XmlBehaviour,
		g = a("./folding/xml").FoldMode,
		h = function() {
			this.HighlightRules = e, this.$behaviour = new f, this.foldingRules = new g
		};
	c.inherits(h, d),
	function() {
		this.blockComment = {
...
}), ace.define("ace/mode/xml_highlight_rules", ["require", "exports", "module", "ace/lib/oop", "ace/mode/xml_util", "ace/mode/text_highlight_rules"], function(a, b) {
	var c = a("../lib/oop"),
		d = (a("./xml_util"), a("./text_highlight_rules").TextHighlightRules),
		e = function() {
			this.$rules = {
				start: [{
	...
	(function() {
		this.embedTagRules = function(a, b, c) {
			this.$rules.tag.unshift({
				token: ["meta.tag.punctuation.begin", "meta.tag.name." + c],
				regex: "(<)(" + c + ")",
				next: [{
					include: "space"
				}, {
...
}), ace.define("ace/mode/xml_util", ["require", "exports", "module"], function(a, b) {
	function c(a) {
		return [{
			token: "string",
			regex: '"',
	...
	b.tag = function(a, b, e, f) {
		a[b] = [{
			...
			regex: "",
			next: b + "_embed_attribute_list"
		}], a[b + "_qstring"] = d("'", b + "_embed_attribute_list"), a[b + "_qqstring"] = d('"', b + "_embed_attribute_list"), a[b + "_embed_attribute_list"] = [{
			token: "meta.tag.r",
			regex: "/?>",
			next: e
		}, {
...
}), ace.define("ace/mode/behaviour/xml", ["require", "exports", "module", "ace/lib/oop", "ace/mode/behaviour", "ace/mode/behaviour/cstyle", "ace/token_iterator"], function(a, b) {
	function c(a, b) {
		var c = a.type.split(".");
		return b.split(".").every(function(a) {
	...
	var d = a("../../lib/oop"),
		...
		g = a("../../token_iterator").TokenIterator,
		h = function() {
			this.inherit(f, ["string_dquotes"]), this.add("autoclosing", "insertion", function(a, b, d, e, f) {
				if (">" == f) {
					var h = d.getCursorPosition(),
						i = new g(e, h.row, h.column),
						j = i.getCurrentToken();
					if (j && c(j, "string") && i.getCurrentTokenColumn() + j.value.length > h.column) return;
					var k = !1;
					if (j && (c(j, "meta.tag") || c(j, "text") && j.value.match("/"))) k = !0;
					else
						do j = i.stepBackward(); while (j && (c(j, "string") || c(j, "keyword.operator") || c(j, "entity.attribute-name") || c(j, "text"))); if (!j || !c(j, "meta.tag.name") || i.stepBackward().value.match("/")) return;
					var l = j.value;
					if (k) var l = l.substring(0, h.column - j.start);
					return {
						text: "></" + l + ">",
						selection: [1, 1]
					}
				...
				if ("\n" == e) {
					...
					if ("</" == h) {
						...
						return {
							text: "\n" + j + "\n" + i,
							selection: [1, j.length, 1, j.length]
						}
					}
				}
			})
		};
	d.inherits(h, e), b.XmlBehaviour = h
}), ace.define("ace/mode/folding/xml", ["require", "exports", "module", "ace/lib/oop", "ace/lib/lang", "ace/range", "ace/mode/folding/fold_mode", "ace/token_iterator"], function(a, b) {
	var c = a("../../lib/oop"),
		d = a("../../lib/lang"),
		e = a("../../range").Range,
		f = a("./fold_mode").FoldMode,
		g = a("../../token_iterator").TokenIterator,
		h = b.FoldMode = function(a) {
			f.call(this), this.voidElements = a || {}
		};
	c.inherits(h, f),
	function() {
		this.getFoldWidget = function(a, b, c) {
			var d = this._getFirstTagInLine(a, c);
			return d.closing ? "markbeginend" == b ? "end" : "" : !d.tagName || this.voidElements[d.tagName.toLowerCase()] ? "" : d.selfClosing ? "" : -1 !== d.value.indexOf("/" + d.tagName) ? "" : "start"
		}, this._getFirstTagInLine = function(a, b) {
			for (var c = a.getTokens(b), e = "", f = 0; f < c.length; f++) {
				var g = c[f];
				e += 0 === g.type.lastIndexOf("meta.tag", 0) ? g.value : d.stringRepeat(" ", g.value.length)
			}
			return this._parseTag(e)
		}, this.tagRe = /^(\s*)(<?(\/?)([-_a-zA-Z0-9:!]*)\s*(\/?)>?)/, this._parseTag = function(a) {
			var b = a.match(this.tagRe),
				c = 0;
			return {
				value: a,
				match: b ? b[2] : "",
			...
			var c, d = "";
			do
				if (0 === b.type.lastIndexOf("meta.tag", 0)) {
					if (!c) var c = {
						row: a.getCurrentTokenRow(),
						column: a.getCurrentTokenColumn()
					};
					if (d += b.value, -1 !== d.indexOf(">")) {
						var e = this._parseTag(d);
						return e.start = c, e.end = {
							row: a.getCurrentTokenRow(),
			...
			var c, d = "";
			do
				if (0 === b.type.lastIndexOf("meta.tag", 0) && (c || (c = {
					row: a.getCurrentTokenRow(),
					column: a.getCurrentTokenColumn() + b.value.length
				}), d = b.value + d, -1 !== d.indexOf("<"))) {
					var e = this._parseTag(d);
					return e.end = c, e.start = {
						row: a.getCurrentTokenRow(),
						column: a.getCurrentTokenColumn()
					}, a.stepBackward(), e
				}
			...
			var f, h = d.closing || d.selfClosing,
				i = [];
			if (h)
				for (var j = new g(a, c, d.column + d.match.length), k = {
					row: c,
					column: d.column
					...
					if (f.closing) i.push(f);
					else if (this._pop(i, f), 0 == i.length) return f.start.column += f.tagName.length + 2, e.fromPoints(f.start, k)
				} else
					for (var j = new g(a, c, d.column), l = {
						row: c,
						column: d.column + d.tagName.length + 2
					}; f = this._readTagForward(j);) {
						if (f.selfClosing) {
							if (i.length) continue;
							return f.start.column += f.tagName.length + 2, f.end.column -= 2, e.fromPoints(f.start, f.end)
						}
...
}), ace.define("ace/mode/html", ["require", "exports", "module", "ace/lib/oop", "ace/mode/text", "ace/mode/javascript", "ace/mode/css", "ace/tokenizer", "ace/mode/html_highlight_rules", "ace/mode/behaviour/html", "ace/mode/folding/html", "ace/mode/html_completions"], function(a, b) {
	var c = a("../lib/oop"),
		d = a("./text").Mode,
		e = a("./javascript").Mode,
		f = a("./css").Mode,
		g = (a("../tokenizer").Tokenizer, a("./html_highlight_rules").HtmlHighlightRules),
		h = a("./behaviour/html").HtmlBehaviour,
		i = a("./folding/html").FoldMode,
		j = a("./html_completions").HtmlCompletions,
		k = function() {
			this.HighlightRules = g, this.$behaviour = new h, this.$completer = new j, this.createModeDelegates({
				"js-": e,
				"css-": f
...
}), ace.define("ace/mode/css", ["require", "exports", "module", "ace/lib/oop", "ace/mode/text", "ace/tokenizer", "ace/mode/css_highlight_rules", "ace/mode/matching_brace_outdent", "ace/worker/worker_client", "ace/mode/behaviour/css", "ace/mode/folding/cstyle"], function(a, b) {
	var c = a("../lib/oop"),
		d = a("./text").Mode,
		e = (a("../tokenizer").Tokenizer, a("./css_highlight_rules").CssHighlightRules),
		f = a("./matching_brace_outdent").MatchingBraceOutdent,
		g = a("../worker/worker_client").WorkerClient,
		h = a("./behaviour/css").CssBehaviour,
		i = a("./folding/cstyle").FoldMode,
		j = function() {
			this.HighlightRules = e, this.$outdent = new f, this.$behaviour = new h, this.foldingRules = new i
		};
	...
	function() {
		this.foldingRules = "cStyle", this.blockComment = {
			...
			var d = this.$getIndent(b),
				e = this.getTokenizer().getLineTokens(b, a).tokens;
...
}), ace.define("ace/mode/css_highlight_rules", ["require", "exports", "module", "ace/lib/oop", "ace/lib/lang", "ace/mode/text_highlight_rules"], function(a, b) {
	var c = a("../lib/oop"),
		d = (a("../lib/lang"), a("./text_highlight_rules").TextHighlightRules),
		...
		f = b.supportFunction = "rgb|rgba|url|attr|counter|counters",
		g = b.supportConstant = "absolute|after-edge|after|all-scroll|all|alphabetic|always|antialiased|armenian|auto|avoid-column|avoid-page|avoid|balance|baseline|before-edge|before|below|bidi-override|block-line-height|block|bold|bolder|border-box|both|bottom|box|break-all|break-word|capitalize|caps-height|caption|center|central|char|circle|cjk-ideographic|clone|close-quote|col-resize|collapse|column|consider-shifts|contain|content-box|cover|crosshair|cubic-bezier|dashed|decimal-leading-zero|decimal|default|disabled|disc|disregard-shifts|distribute-all-lines|distribute-letter|distribute-space|distribute|dotted|double|e-resize|ease-in|ease-in-out|ease-out|ease|ellipsis|end|exclude-ruby|fill|fixed|georgian|glyphs|grid-height|groove|hand|hanging|hebrew|help|hidden|hiragana-iroha|hiragana|horizontal|icon|ideograph-alpha|ideograph-numeric|ideograph-parenthesis|ideograph-space|ideographic|inactive|include-ruby|inherit|initial|inline-block|inline-box|inline-line-height|inline-table|inline|inset|inside|inter-ideograph|inter-word|invert|italic|justify|katakana-iroha|katakana|keep-all|last|left|lighter|line-edge|line-through|line|linear|list-item|local|loose|lower-alpha|lower-greek|lower-latin|lower-roman|lowercase|lr-tb|ltr|mathematical|max-height|max-size|medium|menu|message-box|middle|move|n-resize|ne-resize|newspaper|no-change|no-close-quote|no-drop|no-open-quote|no-repeat|none|normal|not-allowed|nowrap|nw-resize|oblique|open-quote|outset|outside|overline|padding-box|page|pointer|pre-line|pre-wrap|pre|preserve-3d|progress|relative|repeat-x|repeat-y|repeat|replaced|reset-size|ridge|right|round|row-resize|rtl|s-resize|scroll|se-resize|separate|slice|small-caps|small-caption|solid|space|square|start|static|status-bar|step-end|step-start|steps|stretch|strict|sub|super|sw-resize|table-caption|table-cell|table-column-group|table-column|table-footer-group|table-header-group|table-row-group|table-row|table|tb-rl|text-after-edge|text-before-edge|text-bottom|text-size|text-top|text|thick|thin|transparent|underline|upper-alpha|upper-latin|upper-roman|uppercase|use-script|vertical-ideographic|vertical-text|visible|w-resize|wait|whitespace|z-index|zero",
		h = b.supportConstantColor = "aqua|black|blue|fuchsia|gray|green|lime|maroon|navy|olive|orange|purple|red|silver|teal|white|yellow",
		i = b.supportConstantFonts = "arial|century|comic|courier|garamond|georgia|helvetica|impact|lucida|symbol|system|tahoma|times|trebuchet|utopia|verdana|webdings|sans-serif|serif|monospace",
		j = b.numRe = "\\-?(?:(?:[0-9]+)|(?:[0-9]*\\.[0-9]+))",
		k = b.pseudoElements = "(\\:+)\\b(after|before|first-letter|first-line|moz-selection|selection)\\b",
		l = b.pseudoClasses = "(:)\\b(active|checked|disabled|empty|enabled|first-child|first-of-type|focus|hover|indeterminate|invalid|last-child|last-of-type|link|not|nth-child|nth-last-child|nth-last-of-type|nth-of-type|only-child|only-of-type|required|root|target|valid|visited)\\b",
		m = function() {
			var a = this.createKeywordMapper({
				"support.function": f,
				"support.constant": g,
...
}), ace.define("ace/mode/behaviour/css", ["require", "exports", "module", "ace/lib/oop", "ace/mode/behaviour", "ace/mode/behaviour/cstyle", "ace/token_iterator"], function(a, b) {
	var c = a("../../lib/oop"),
		d = (a("../behaviour").Behaviour, a("./cstyle").CstyleBehaviour),
		e = a("../../token_iterator").TokenIterator,
		f = function() {
			this.inherit(d), this.add("colon", "insertion", function(a, b, c, d, f) {
				if (":" === f) {
					var g = c.getCursorPosition(),
						h = new e(d, g.row, g.column),
						i = h.getCurrentToken();
					if (i && i.value.match(/\s+/) && (i = h.stepBackward()), i && "support.type" === i.type) {
						var j = d.doc.getLine(g.row),
							k = j.substring(g.column, g.column + 1);
						if (":" === k) return {
							text: "",
							selection: [1, 1]
						};
						if (!j.substring(g.column).match(/^\s*;/)) return {
							text: ":;",
							selection: [1, 1]
						}
					}
				...
				if (!f.isMultiLine() && ":" === g) {
					var h = c.getCursorPosition(),
						i = new e(d, h.row, h.column),
						j = i.getCurrentToken();
					if (j && j.value.match(/\s+/) && (j = i.stepBackward()), j && "support.type" === j.type) {
						var k = d.doc.getLine(f.start.row),
							l = k.substring(f.end.column, f.end.column + 1);
				...
				if (";" === e) {
					var f = c.getCursorPosition(),
						...
						h = g.substring(f.column, f.column + 1);
					if (";" === h) return {
						text: "",
						selection: [1, 1]
					}
...
}), ace.define("ace/mode/html_highlight_rules", ["require", "exports", "module", "ace/lib/oop", "ace/lib/lang", "ace/mode/css_highlight_rules", "ace/mode/javascript_highlight_rules", "ace/mode/xml_highlight_rules"], function(a, b) {
	var c = a("../lib/oop"),
		d = a("../lib/lang"),
		e = a("./css_highlight_rules").CssHighlightRules,
		...
		i = function() {
			g.call(this), this.addRules({
				...
				tag: [{
					token: function(a, b) {
						var c = h[b];
						return ["meta.tag.punctuation.begin", "meta.tag.name" + (c ? "." + c : "")]
					},
					regex: "(<)([-_a-zA-Z0-9:]+)",
...
}), ace.define("ace/mode/behaviour/html", ["require", "exports", "module", "ace/lib/oop", "ace/mode/behaviour/xml", "ace/mode/behaviour/cstyle", "ace/token_iterator"], function(a, b) {
	function c(a, b) {
		var c = a.type.split(".");
		return b.split(".").every(function(a) {
			return -1 !== c.indexOf(a)
	...
	var d = a("../../lib/oop"),
		...
		f = (a("./cstyle").CstyleBehaviour, a("../../token_iterator").TokenIterator),
		g = ["area", "base", "br", "col", "command", "embed", "hr", "img", "input", "keygen", "link", "meta", "param", "source", "track", "wbr"],
		h = function() {
			this.inherit(e), this.add("autoclosing", "insertion", function(a, b, d, e, h) {
				if (">" == h) {
					var i = d.getCursorPosition(),
						j = new f(e, i.row, i.column),
						k = j.getCurrentToken();
					if (k && c(k, "string") && j.getCurrentTokenColumn() + k.value.length > i.column) return;
					var l = !1;
					if (k && (c(k, "meta.tag") || c(k, "text") && k.value.match("/"))) l = !0;
					else
						do k = j.stepBackward(); while (k && (c(k, "string") || c(k, "keyword.operator") || c(k, "entity.attribute-name") || c(k, "text"))); if (!k || !c(k, "meta.tag.name") || j.stepBackward().value.match("/")) return;
					var m = k.value;
					if (l) var m = m.substring(0, i.column - k.start);
					if (-1 !== g.indexOf(m)) return;
					return {
						text: "></" + m + ">",
						selection: [1, 1]
					}
	...
	d.inherits(h, e), b.HtmlBehaviour = h
}), ace.define("ace/mode/folding/html", ["require", "exports", "module", "ace/lib/oop", "ace/mode/folding/mixed", "ace/mode/folding/xml", "ace/mode/folding/cstyle"], function(a, b) {
	var c = a("../../lib/oop"),
		d = a("./mixed").FoldMode,
		e = a("./xml").FoldMode,
		f = a("./cstyle").FoldMode,
	...
	c.inherits(g, d)
}), ace.define("ace/mode/folding/mixed", ["require", "exports", "module", "ace/lib/oop", "ace/mode/folding/fold_mode"], function(a, b) {
	var c = a("../../lib/oop"),
		d = a("./fold_mode").FoldMode,
		e = b.FoldMode = function(a, b) {
	...
	function() {
		this.$getMode = function(a) {
			...
			for (var b in this.subModes)
				if (0 === a.indexOf(b)) return this.subModes[b];
			return null
...
}), ace.define("ace/mode/html_completions", ["require", "exports", "module", "ace/token_iterator"], function(a, b) {
	function c(a, b) {
		var c = a.type.split(".");
		return b.split(".").every(function(a) {
			return -1 !== c.indexOf(a)
	...
	function d(a, b) {
		var d = new e(a, b.row, b.column),
			f = d.getCurrentToken();
		if (!(f && (c(f, "tag") || c(f, "text") && f.value.match("/"))))
			do f = d.stepBackward(); while (f && (c(f, "string") || c(f, "operator") || c(f, "attribute-name") || c(f, "text")));
		return f && c(f, "tag-name") && !d.stepBackward().value.match("/") ? f.value : void 0
	}
	var e = a("../token_iterator").TokenIterator,
		f = ["accesskey", "class", "contenteditable", "contextmenu", "dir", "draggable", "dropzone", "hidden", "id", "lang", "spellcheck", "style", "tabindex", "title", "translate"],
	...
	(function() {
		this.getCompletions = function(a, b, d, e) {
			...
			return f ? c(f, "tag-name") || "<" == f.value && c(f, "text") ? this.getTagCompletions(a, b, d, e) : c(f, "text") || c(f, "attribute-name") ? this.getAttributeCompetions(a, b, d, e) : [] : []
		}, this.getTagCompletions = function(a, b, c, d) {
			var e = j;
			return d && (e = e.filter(function(a) {
		...
		}, this.getAttributeCompetions = function(a, b, c, e) {
			var f = d(b, c);
			if (!f) return [];
			var g = h;
			return f in i && (g = g.concat(i[f])), e && (g = g.filter(function(a) {
...
}), ace.define("ace/mode/markdown_highlight_rules", ["require", "exports", "module", "ace/lib/oop", "ace/lib/lang", "ace/mode/text_highlight_rules", "ace/mode/javascript_highlight_rules", "ace/mode/xml_highlight_rules", "ace/mode/html_highlight_rules", "ace/mode/css_highlight_rules"], function(a, b) {
	var c = a("../lib/oop"),
		d = a("../lib/lang"),
		e = a("./text_highlight_rules").TextHighlightRules,
		f = (a("./javascript_highlight_rules").JavaScriptHighlightRules, a("./xml_highlight_rules").XmlHighlightRules, a("./html_highlight_rules").HtmlHighlightRules),
		g = (a("./css_highlight_rules").CssHighlightRules, function(a) {
			return "(?:[^" + d.escapeRegExp(a) + "\\\\]|\\\\.)*"
		}),
		h = function() {
			f.call(this);
			var a = {
				blockquote: "string.constant.other",
				code: "support.function",
				head: "markup.heading.constant.numeric",
				head2: "markup.heading.heading2.constant.numeric",
				strong: "keyword.strong",
				emphasis: "keyword",
				image: "string.regexp.image",
				notebook: "notebook.string.comment"
			};
			this.$rules.start.unshift({
				...
				token: a.head + ".multi.1",
				regex: "^=+(?=\\s*$)"
			}, {
				token: a.head + ".multi.2",
				regex: /^\-+(?=\s*$)/
			}, {
				token: a.head2,
				regex: /^#{4,6}(?=\s*[^ #]|\s+#.)/,
				...
				basic: [{
					...
					token: [a.image, a.image, a.image, "markup.underline." + a.image, a.image, a.image],
					regex: "(!\\[)(" + g("]") + ')(\\]\\()((?:[^\\)\\s\\\\]|\\\\.|\\s(?=[^"]))*)(\\s*"' + g('"') + '"\\s*)?(\\))'
				}, {
					token: ["string", "string", "string", "markup.underline.string", "string", "string"],
					regex: "(\\[)(" + g("]") + ')(\\]\\()((?:[^\\)\\s\\\\]|\\\\.|\\s(?=[^"]))*)(\\s*"' + g('"') + '"\\s*)?(\\))'
				}, {
					token: a.strong,
					regex: "([*]{2}|[_]{2}(?=\\S))(.*?\\S[*_]*)(\\1)"
	...
	c.inherits(h, e), b.MarkdownHighlightRules = h
}), ace.define("ace/mode/folding/markdown", ["require", "exports", "module", "ace/lib/oop", "ace/mode/folding/fold_mode", "ace/range"], function(a, b) {
	var c = a("../../lib/oop"),
		d = a("./fold_mode").FoldMode,
		e = a("../../range").Range,
		f = b.FoldMode = function() {};
	c.inherits(f, d),
	function() {
		this.foldingStartMarker = /^(?:[=-]+\s*$|#{1,6} |`{3})/, this.getFoldWidget = function(a, b, c) {
			var d = a.getLine(c);
			return this.foldingStartMarker.test(d) ? "`" == d[0] && "start" == a.bgTokenizer.getState(c) ? "end" : "start" : ""
		}, this.getFoldWidgetRange = function(a, b, c) {
			function d(b) {
			...
			if (g.match(this.foldingStartMarker)) {
				...
				if (d(c)) {
					...
					if (k = c - (l && -1 != ["=", "-"].indexOf(l.value[0]) ? 2 : 1), k > j)
						for (; k > j && /^\s*$/.test(a.getLine(k));) k--;
					if (k > j) {
